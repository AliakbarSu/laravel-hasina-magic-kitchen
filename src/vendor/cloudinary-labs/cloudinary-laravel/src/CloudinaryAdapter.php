...
class CloudinaryAdapter implements FilesystemAdapter
{
    ...
    public function writeStream($path, $resource, Config $options): void
    {
        ...
        $newPublicId = $fileExtension
            ? substr($publicId, 0, -(strlen($fileExtension) + 1))
            : $publicId;
        ...
        resolve(CloudinaryEngine::class)->upload(
    ...
    public function rename($path, $newpath)
    {
        ...
        $remoteNewPath =
            $pathInfo['dirname'] != '.'
                ? $newPathInfo['dirname'] . '/' . $newPathInfo['filename']
    ...
    public function setVisibility($path, $visibility): void
    {
        throw UnableToSetVisibility::atLocation(
            $path,
            'Cloudinary API does not support visibility.'
        );
    }
    ...
    public function listContents(
        ...
        do {
            $response = (array) $this->adminApi()->assets([
                ...
                'prefix' => $directory,
                'max_results' => 500,
                'next_cursor' => isset($response['next_cursor'])
        ...
        foreach ($resources as $i => $resource) {
            $resources[$i] = $this->prepareFileAttributes(
                $this->prepareResourceMetadata($resource)
            );
        }
        return $resources;
    }
