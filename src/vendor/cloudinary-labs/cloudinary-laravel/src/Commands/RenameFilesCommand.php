...
class RenameFilesCommand extends Command
{
    ...
    public function handle(CloudinaryEngine $engine)
    {
        if (!config('cloudinary.cloud_url')) {
            $this->warn(
                'Please ensure your Cloudinary credentials are set before continuing.'
            );
        ...
        if (!is_string($fromPublicId) || !is_string($toPublicId)) {
            $this->warn(
                'Please ensure a valid public Id is passed as an argument.'
            );
        ...
        $this->info(
            "About to rename {$fromPublicId} file to {$toPublicId} on Cloudinary..."
        );
        ...
        try {
            ...
            $this->warn(
                "Renaming of file on Cloudinary failed because: {$exception->getMessage()}."
            );
        }
    }
}
