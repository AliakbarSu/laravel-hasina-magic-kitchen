...
abstract class AbstractSyncBuilder
{
     ...
     * @return $this
     */
    public function setSourceFilenameConverter(
        FilenameConverterInterface $converter
    ) {
     ...
     * @return $this
     */
    public function setTargetFilenameConverter(
        FilenameConverterInterface $converter
    ) {
    ...
    public function addRegexFilter($search)
    {
        ...
        $this->sourceIterator = new FilterIterator(
            $this->sourceIterator,
    ...
    public function build()
    {
        ...
        $this->sourceConverter =
            $this->sourceConverter ?: $this->getDefaultSourceConverter();
        $this->targetConverter =
            $this->targetConverter ?: $this->getDefaultTargetConverter();
        ...
        if ($this->debug) {
            $this->addDebugListener(
                $sync,
                is_bool($this->debug) ? STDOUT : $this->debug
            );
        }
    ...
    protected function assertFileIteratorSet()
    {
        ...
        if (!isset($this->sourceIterator)) {
            throw new RuntimeException(
                'A source file iterator must be specified'
            );
        }
    }
    ...
    protected function addCustomParamListener(HasDispatcherInterface $sync)
    {
        ...
        $sync
            ->getEventDispatcher()
            ->addListener(UploadSync::BEFORE_TRANSFER, function (Event $e) use (
                $params
            ) {
    ...
    protected function createS3Iterator()
    {
        ...
        return $this->filterIterator(
            new \NoRewindIterator(new OpendirIterator($dh, $dir))
        );
    }
}
