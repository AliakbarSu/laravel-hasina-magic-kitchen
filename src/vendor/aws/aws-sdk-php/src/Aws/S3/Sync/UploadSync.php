    ...
    protected function createTransferAction(\SplFileInfo $file)
    {
        ...
        if (!($resource = fopen($filename, 'r'))) {
            ...
            throw new RuntimeException(
                'Could not open ' . $file->getPathname() . ' for reading'
            );
        ...
        if (
            $body->getWrapper() == 'plainfile' &&
            $file->getSize() >= $this->options['multipart_upload_size']
        ) {
        ...
        return $this->options['client']->getCommand('PutObject', [
