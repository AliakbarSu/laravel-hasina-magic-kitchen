...
class UploadSyncBuilder extends AbstractSyncBuilder
{
    ...
    protected function addCustomParamListener(HasDispatcherInterface $sync)
    {
        ...
        $sync
            ->getEventDispatcher()
            ->addListener(UploadSync::BEFORE_MULTIPART_BUILD, function (
                Event $e
            ) use ($params) {
        ...
        $sync
            ...
            ->addListener(UploadSync::BEFORE_TRANSFER, function (Event $e) use (
                ...
                if ($c instanceof CommandInterface) {
                    ...
                    fwrite(
                        $resource,
                        "Uploading {$uri} -> {$c['Key']} ({$size} bytes)\n"
                    );
                    return;
                }
                ...
                fwrite(
                    $resource,
                    'Beginning multipart upload: ' . $body->getUri() . ' -> '
                );
                fwrite(
                    $resource,
                    $c->getState()->getFromId('Key') . " ({$totalSize} bytes)\n"
                );
                ...
                $c->getEventDispatcher()->addListener(
                    ...
                    function ($e) use (&$progress, $totalSize, $resource) {
                        ...
                        fwrite(
                            $resource,
                            "- Part {$command['PartNumber']} ({$size} bytes, {$percentage}%)\n"
                        );
                        $progress += $size;
                    }
                );
