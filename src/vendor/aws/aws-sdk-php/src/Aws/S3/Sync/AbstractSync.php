...
abstract class AbstractSync extends AbstractHasDispatcher
{
    ...
    public function __construct(array $options)
    {
        $this->options = Collection::fromConfig(
            $options,
            ['concurrency' => 10],
            ['client', 'bucket', 'iterator', 'source_converter']
        );
        $this->init();
    }
    ...
    {
        return [self::BEFORE_TRANSFER, self::AFTER_TRANSFER];
    }
    ...
    protected function transferFiles(array $files)
    {
        ...
        $event = ['sync' => $this, 'client' => $this->options['client']];

        $commands = [];
        foreach ($files as $file) {
            if ($action = $this->createTransferAction($file)) {
                $event = ['command' => $action, 'file' => $file] + $event;
