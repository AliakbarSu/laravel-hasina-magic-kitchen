...
class DownloadSyncBuilder extends AbstractSyncBuilder
{
    ...
    protected function getTargetIterator()
    {
        ...
        if (!is_dir($this->directory) && !mkdir($this->directory, 0777, true)) {
            ...
            throw new RuntimeException(
                'Unable to create root download directory: ' . $this->directory
            );
    ...
    {
        $this->sourceIterator =
            $this->sourceIterator ?: $this->createS3Iterator();
    }
        ...
        $sync
            ...
            ->addListener(UploadSync::BEFORE_TRANSFER, function (Event $e) use (
                ...
                if ($e['command'] instanceof CommandInterface) {
                    $from =
                        $e['command']['Bucket'] . '/' . $e['command']['Key'];
                    $to =
                        $e['command']['SaveAs'] instanceof EntityBodyInterface
