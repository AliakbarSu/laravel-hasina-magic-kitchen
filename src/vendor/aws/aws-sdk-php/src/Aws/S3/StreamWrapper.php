...
class StreamWrapper
{
    ...
    public function stream_open($path, $mode, $options, &$opened_path)
    {
        ...
        if (!$params['Key']) {
            $errors[] =
                'Cannot open a bucket. You must specify a path in the form of s3://bucket/key';
        }

        if (strpos($mode, '+')) {
            $errors[] =
                'The Amazon S3 stream wrapper does not allow simultaneous reading and writing.';
        }
    ...
    public function url_stat($path, $flags)
    {
        ...
        if (!$parts['Key']) {
            ...
            if (
                !$parts['Bucket'] ||
                static::$client->doesBucketExist($parts['Bucket'])
            ) {
        ...
        try {
            try {
                ...
                $result = static::$client->listObjects([
                    'Bucket' => $parts['Bucket'],
    ...
    public function rmdir($path, $options)
    {
        ...
        if (!$params['Bucket']) {
            return $this->triggerError(
                'You cannot delete s3://. Please specify a bucket.'
            );
        }

        try {
            if (!$params['Key']) {
                static::$client->deleteBucket(['Bucket' => $params['Bucket']]);
                $this->clearStatInfo($path);
                return true;
            }
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $objectIterator = static::$client->getIterator(
            'ListObjects',
            $operationParams,
            [
        ...
        $this->objectIterator = new FilterIterator($objectIterator, function (
            $key
        ) use ($filterFn) {
    ...
    protected function openReadStream(array $params, array &$errors)
    {
        ...
        $request = $this->getSignedRequest(
            static::$client->getCommand('GetObject', $params)
        );
        // Create a stream that uses the EntityBody object
        $factory =
            $this->getOption('stream_factory') ?: new PhpStreamRequestFactory();
        $this->body = $factory->fromRequest(
            $request,
            [],
    ...
    protected function formatUrlStat($result = null)
    {
        ...
        if ($type == 'NULL' || $type == 'string') {
            ...
            $stat['mtime'] = $stat[9] = $stat['ctime'] = $stat[10] = strtotime(
                $result['LastModified']
            );
    ...
    private function createPseudoDirectory($path, array $params)
    {
        ...
        if (
            static::$client->doesObjectExist($params['Bucket'], $params['Key'])
        ) {
            return $this->triggerError("Directory already exists: {$path}");
        }
