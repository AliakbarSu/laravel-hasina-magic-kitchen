...
class BucketStyleListener implements EventSubscriberInterface
{
    ...
    {
        return ['command.after_prepare' => ['onCommandAfterPrepare', -255]];
    }
    ...
    public function onCommandAfterPrepare(Event $event)
    {
        ...
        if (
            !$command['PathStyle'] &&
            $command->getClient()->isValidBucketName($bucket) &&
            !(
                $command->getRequest()->getScheme() == 'https' &&
                strpos($bucket, '.')
            )
        ) {
            ...
            $request->setPath(
                preg_replace("#^/{$bucket}#", '', $request->getPath())
            );
        } else {
            $pathStyle = true;
        }
