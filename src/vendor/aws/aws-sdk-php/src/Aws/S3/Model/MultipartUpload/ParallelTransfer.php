...
class ParallelTransfer extends AbstractTransfer
{
    ...
    protected function init()
    {
        ...
        if (
            !$this->source->isLocal() ||
            $this->source->getWrapper() != 'plainfile'
        ) {
            throw new RuntimeException(
                'The source data must be a local file stream when uploading in parallel.'
            );
        }

        if (empty($this->options['concurrency'])) {
            throw new RuntimeException(
                'The `concurrency` option must be specified when instantiating.'
            );
        }
    }
    ...
    {
        $totalParts = (int) ceil(
            $this->source->getContentLength() / $this->partSize
        );
        ...
        while (!$this->stopped && count($this->state) < $totalParts) {
            ...
            for (
                $i = 0;
                $i < $concurrency && $i + $currentTotal < $totalParts;
                $i++
            ) {
                // Move the offset to the correct position
                $partsToSend[$i]->setOffset(
                    ($currentTotal + $i) * $this->partSize
                );
                ...
                $eventData['command'] = $this->client->getCommand(
                    'UploadPart',
    ...
    protected function prepareParts($concurrency)
    {
        ...
        $parts = [new ReadLimitEntityBody($this->source, $this->partSize)];
        // Open EntityBody handles for each part to upload in parallel
        for ($i = 1; $i < $concurrency; $i++) {
            $parts[] = new ReadLimitEntityBody(
                new EntityBody(fopen($url, 'r')),
                $this->partSize
            );
        }
