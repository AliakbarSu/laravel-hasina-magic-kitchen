...
class UploadBuilder extends AbstractUploadBuilder
{
    ...
    {
        $this->minPartSize = (int) max(
            (int) $minSize,
            AbstractTransfer::MIN_PART_SIZE
        );
    ...
    public function build()
    {
        if ($this->state instanceof TransferState) {
            $this->commandOptions = array_replace(
                $this->commandOptions,
        ...
        if (
            !isset($this->commandOptions['Bucket']) ||
            !isset($this->commandOptions['Key']) ||
            !$this->client ||
            !$this->source
        ) {
            throw new InvalidArgumentException(
                'You must specify a Bucket, Key, client, and source.'
            );
        }

        if ($this->state && !$this->source->isSeekable()) {
            throw new InvalidArgumentException(
                'You cannot resume a transfer using a non-seekable source.'
            );
        }

        // If no state was set, then create one by initiating or loading a multipart upload
        if (is_string($this->state)) {
            $this->state = TransferState::fromUploadId(
                $this->client,
                UploadId::fromParams([
    ...
    protected function initiateMultipartUpload()
    {
        ...
        $result = $this->client
            ->getCommand('CreateMultipartUpload', $params)
            ->execute();
