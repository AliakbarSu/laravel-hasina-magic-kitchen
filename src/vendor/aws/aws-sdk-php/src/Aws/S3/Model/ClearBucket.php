...
class ClearBucket extends AbstractHasDispatcher
{
    ...
    {
        return [self::AFTER_DELETE, self::BEFORE_CLEAR, self::AFTER_CLEAR];
    }
    ...
    {
        if (!$this->iterator) {
            $this->iterator = $this->client->getIterator('ListObjectVersions', [
                'Bucket' => $this->bucket,
            ]);
        }
    ...
    {
        $that = $this;
        $batch = DeleteObjectsBatch::factory(
            $this->client,
            $this->bucket,
            $this->mfa
        );
        $batch = new NotifyingBatch($batch, function ($items) use ($that) {
            $that->dispatch(ClearBucket::AFTER_DELETE, ['keys' => $items]);
        });
        ...
        $this->dispatch(self::BEFORE_CLEAR, [
        ...
        $this->dispatch(self::AFTER_CLEAR, ['deleted' => $deleted]);
