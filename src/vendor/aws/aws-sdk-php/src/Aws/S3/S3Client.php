...
class S3Client extends AbstractClient
{
     ...
     */
    protected static $commandAliases = [
    ...
    public static function factory($config = [])
    {
        ...
        if (!isset($config[Options::BACKOFF])) {
            $retries = isset($config[Options::BACKOFF_RETRIES])
                ...
                : 3;
            $config[Options::BACKOFF] = static::createBackoffPlugin(
                $exceptionParser,
                $retries
            );
        }
            ...
            ->setIteratorsConfig([
                ...
                'operations' => [
                    ...
                    'ListMultipartUploads' => [
                        ...
                        'token_param' => ['KeyMarker', 'UploadIdMarker'],
                        'token_key' => ['NextKeyMarker', 'NextUploadIdMarker'],
                    ],
                    'ListObjects' => [
                        'limit_param' => 'MaxKeys',
                        'token_param' => 'Marker',
                    ...
                    'ListObjectVersions' => [
                        'limit_param' => 'MaxKeys',
                        'token_param' => ['KeyMarker', 'VersionIdMarker'],
                        'token_key' => ['nextKeyMarker', 'nextVersionIdMarker'],
                    ],
                    'ListParts' => [
        ...
        $client->addSubscriber(
            new UploadBodyListener(['PutObject', 'UploadPart'])
        );
     ...
     */
    private static function createBackoffPlugin(
        S3ExceptionParser $exceptionParser,
        $retries = 3
    ) {
    ...
    {
        $currentValue = isset($config[Options::SIGNATURE])
            ? $config[Options::SIGNATURE]
            : null;
    ...
    public function createPresignedUrl(RequestInterface $request, $expires)
    {
        if ($request->getClient() !== $this) {
            throw new InvalidArgumentException(
                'The request object must be associated with the client. Use the ' .
                    '$client->get(), $client->head(), $client->post(), $client->put(), etc. methods when passing in a ' .
                    'request object'
            );
        }

        return $this->signature->createPresignedUrl(
            $request,
            $this->credentials,
            $expires
        );
    ...
    public function deleteMatchingObjects(
        ...
        if (!$prefix && !$regex) {
            throw new RuntimeException(
                'A prefix or regex is required, or use S3Client::clearBucket().'
            );
        }
        ...
        if ($regex) {
            $iterator = new FilterIterator($iterator, function ($current) use (
                $regex
            ) {
     ...
     */
    protected function checkExistenceWithCommand(
