...
class CloudFrontClient extends AbstractClient
{
    ...
    public static function factory($config = [])
    {
        ...
        if (
            isset($config[Options::VERSION]) &&
            $config[Options::VERSION] < self::LATEST_API_VERSION
        ) {
        ...
        return ClientBuilder::factory(__NAMESPACE__)
            ...
            ->setConfigDefaults([
                ...
                Options::SERVICE_DESCRIPTION =>
                    __DIR__ . '/Resources/cloudfront-%s.php',
            ])
    ...
    public function getSignedUrl(array $options)
    {
        if (!extension_loaded('openssl')) {
            ...
            throw new RequiredExtensionNotLoadedException(
                'The openssl extension is required to sign CloudFront urls.'
            );
    ...
    protected function createCannedPolicy($scheme, $url, $expires)
    {
        if (!$expires) {
            throw new InvalidArgumentException(
                'An expires option is required when using a canned policy'
            );
        }
        ...
        if ($scheme == 'http' || $scheme == 'https') {
            ...
            throw new InvalidArgumentException(
                "Invalid URI scheme: {$scheme}. Must be one of http or rtmp."
            );
        }
