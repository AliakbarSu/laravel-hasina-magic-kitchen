...
class LogFileIterator extends \IteratorIterator
{
    ...
    public static function forTrail(
        ...
        if (!$s3BucketName) {
            ...
            throw new InvalidArgumentException(
                'The bucket name could not be determined from the trail.',
                0,
                $prev
            );
        }
    ...
    private function buildListObjectsIterator(array $options)
    {
        ...
        $candidatePrefix = ltrim(
            strtr(self::PREFIX_TEMPLATE, $keyPrefixParts),
            '/'
        );
        $logKeyPrefix = $candidatePrefix;
        if (
            ($index = strpos($candidatePrefix, self::PREFIX_WILDCARD)) !== false
        ) {
    ...
    private function normalizeDateValue($date)
    {
        ...
        if (is_string($date)) {
            ...
            throw new \InvalidArgumentException(
                'Date values must be a string, an int, or a DateTime object.'
            );
        }
    ...
    private function applyRegexFilter(
        ...
        if ($logKeyPrefix !== $candidatePrefix) {
            ...
            if ($logKeyPrefix !== $regex) {
                ...
                $objectsIterator = new FilterIterator(
                    $objectsIterator,
    ...
    private function applyDateFilter($objectsIterator, $startDate, $endDate)
    {
        ...
        if ($startDate || $endDate) {
            $objectsIterator = new FilterIterator($objectsIterator, function (
                $object
            ) use ($startDate, $endDate) {
                if (
                    preg_match('/[0-9]{8}T[0-9]{4}Z/', $object['Key'], $matches)
                ) {
