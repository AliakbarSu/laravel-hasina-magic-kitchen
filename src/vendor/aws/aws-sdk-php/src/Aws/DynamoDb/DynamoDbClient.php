...
class DynamoDbClient extends AbstractClient
{
        ...
        if (!isset($config[Options::BACKOFF])) {
            ...
            $config[Options::BACKOFF] = new BackoffPlugin(
                ...
                new Crc32ErrorChecker(
                    self::createDynamoDbBackoffStrategy(
                        $exceptionParser,
                        $retries
                    )
                )
        ...
        return ClientBuilder::factory(__NAMESPACE__)
            ...
            ->setConfigDefaults([
                ...
                Options::SERVICE_DESCRIPTION =>
                    __DIR__ . '/Resources/dynamodb-%s.php',
     ...
     * @internal
     */
    public static function createDynamoDbBackoffStrategy(
        JsonQueryExceptionParser $exceptionParser,
        $retries = 11
    ) {
        // Retry failed requests up to 11 times instead of the normal 3
        return new TruncatedBackoffStrategy(
            $retries,
    ...
    {
        $config = array_replace(['dynamodb_client' => $this], $config);
