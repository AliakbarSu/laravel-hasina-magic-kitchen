...
class WriteRequestBatch extends AbstractBatchDecorator
{
    ...
    public function add($item)
    {
        if ($item instanceof AbstractCommand) {
            ...
            $name = $item->getName();
            if (in_array($name, ['PutItem', 'DeleteItem'])) {
                $class =
                    __NAMESPACE__ .
                    '\\' .
                    str_replace('Item', 'Request', $name);
                $item = $class::fromCommand($item);
            } else {
                throw new InvalidArgumentException(
                    'The command provided was not a PutItem or DeleteItem command.'
                );
            }
        }
        ...
        if (!($item instanceof WriteRequestInterface)) {
            throw new InvalidArgumentException(
                'The item you are trying to add to the batch queue is invalid.'
            );
        }
