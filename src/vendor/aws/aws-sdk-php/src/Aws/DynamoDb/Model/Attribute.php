...
class Attribute implements ToArrayInterface
{
    ...
    public static function factory($value, $depth = 0)
    {
        ...
        if ($depth > 1) {
            throw new InvalidArgumentException(
                'Sets must be at most one level deep.'
            );
        }
        ...
        if ($value === null || $value === [] || $value === '') {
            ...
            throw new InvalidArgumentException(
                'The value must be able to be converted to string.'
            );
        }
        ...
        if (is_int($value) || is_float($value)) {
            ...
            foreach ($value as $subValue) {
                ...
                if ($setType === null) {
                    ...
                    throw new InvalidArgumentException(
                        'The set did not contain values of a uniform type.'
                    );
                }
    ...
    public function getFormatted($format = Attribute::FORMAT_PUT)
    {
        switch ($format) {
            ...
            case self::FORMAT_UPDATE:
                $formatted = ['Value' => [$this->type => $this->value]];
                break;
    ...
    public function setType($type)
    {
        if (in_array($type, Type::values())) {
            ...
            throw new InvalidArgumentException(
                'An attribute type must be a valid DynamoDB type.'
            );
        }
    ...
    public function setValue($value)
    {
        if (is_string($value) || is_array($value)) {
            ...
            throw new InvalidArgumentException(
                'An attribute value may only be a string or array.'
            );
        }
