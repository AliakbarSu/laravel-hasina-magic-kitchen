...
class SessionHandler implements SessionHandlerInterface
{
    ...
    public function createSessionsTable($readCapacityUnits, $writeCapacityUnits)
    {
        ...
        $this->client->waitUntil('table_exists', ['TableName' => $tableName]);
    ...
    public function destroy($id)
    {
        ...
        $this->sessionWritten = $this->lockingStrategy->doDestroy(
            $this->formatId($id)
        );
    ...
    public function garbageCollect()
    {
        ...
        $scanParams = [
            'TableName' => $tableName,
            'AttributesToGet' => [$this->config->get('hash_key')],
            'ScanFilter' => [
                'expires' => [
        ...
        if (isset($delayFunction)) {
            $tableScanner
                ->getEventDispatcher()
                ->addListener('resource_iterator.after_send', $delayFunction);
        }
        ...
        foreach ($tableScanner as $item) {
            ...
            $deleteBatch->add(
                new DeleteRequest([$keyName => $item[$hashKey]], $tableName)
            );
