...
class UploadBuilder extends AbstractUploadBuilder
{
    ...
    public function build()
    {
        ...
        if (
            (!($this->state instanceof State) && !$this->vaultName) ||
            !$this->client ||
            !$this->source
        ) {
            throw new InvalidArgumentException(
                'You must specify a vault name, client, and source.'
            );
        }

        if (!$this->source->isSeekable()) {
            throw new InvalidArgumentException(
                'You cannot upload from a non-seekable source.'
            );
        }
        ...
        if (is_string($this->state)) {
            if (!$this->partGenerator) {
                throw new InvalidArgumentException(
                    'You must provide an UploadPartGenerator when resuming an upload.'
                );
            }
    ...
    protected function initiateMultipartUpload()
    {
        ...
        $partGenerator =
            $this->partGenerator ?:
            UploadPartGenerator::factory($this->source, $this->partSize);
