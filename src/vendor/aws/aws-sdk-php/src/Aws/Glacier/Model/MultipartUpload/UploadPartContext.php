...
class UploadPartContext
{
    ...
    public function addData($data)
    {
        ...
        if ($this->size + $size > $this->maxSize) {
            throw new LogicException(
                'You cannot add data that will exceed the maximum size of this upload.'
            );
        }
        ...
        try {
            ...
            throw new LogicException(
                'You cannot add data to a finalized UploadPartContext.',
                0,
                $e
            );
        }
    ...
    {
        if (!$this->uploadPart) {
            $this->uploadPart = UploadPart::fromArray([
