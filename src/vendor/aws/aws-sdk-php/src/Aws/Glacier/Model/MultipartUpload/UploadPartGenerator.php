 ...
 */
class UploadPartGenerator implements
    \Serializable,
    \IteratorAggregate,
    \Countable
{
    ...
    public static function createSingleUploadPart($body)
    {
        ...
        if (count($generator) > 1) {
            ...
            throw new RuntimeException(
                'You cannot create a single upload that is larger than 4 GB.'
            );
    ...
    public function __construct(EntityBodyInterface $body, $partSize)
    {
        ...
        if (!in_array($this->partSize, $validPartSizes)) {
            throw new InvalidArgumentException(
                'The part size must be a megabyte multiplied by a power of 2 and no ' .
        ...
        if (!$body->isSeekable()) {
            throw new InvalidArgumentException(
                'The upload body must be seekable.'
            );
        }
        ...
        if ($this->archiveSize < 1) {
            throw new InvalidArgumentException(
                'The archive size must be at least 1 byte.'
            );
        }
    }
    ...
    public function getUploadPart($partNumber)
    {
        ...
        if (isset($this->uploadParts[$partNumber - 1])) {
            ...
            throw new OutOfBoundsException(
                "An upload part with part number {$partNumber} at index did not exist."
            );
        }
    }
    /**
    ...
    public function getRootChecksum()
    {
        if (!$this->rootChecksum) {
            $this->rootChecksum = TreeHash::fromChecksums(
                array_map(function (UploadPart $part) {
    ...
    public function unserialize($serialized)
    {
        ...
        // Set properties
        foreach (['uploadParts', 'archiveSize', 'partSize'] as $property) {
            if (isset($data[$property])) {
                $this->{$property} = $data[$property];
            } else {
                throw new RuntimeException(
                    sprintf(
                        'Cannot unserialize the %s class. The %s property is missing.',
                        __CLASS__,
                        $property
                    )
                );
    ...
    protected function generateUploadParts(EntityBodyInterface $body)
    {
        ...
        while (strlen($data) > 0) {
            ...
            if ($uploadContext->isFull()) {
                ...
                $uploadContext = new UploadPartContext(
                    $this->partSize,
                    $body->ftell()
                );
            }
    ...
    protected function updateTotals(UploadPart $part)
    {
        ...
        if ($part->getPartNumber() > self::MAX_NUM_PARTS) {
            ...
            throw new OverflowException(
                'An archive must be uploaded in ' .
                    self::MAX_NUM_PARTS .
                    ' parts or less.'
            );
            // @codeCoverageIgnoreEnd
