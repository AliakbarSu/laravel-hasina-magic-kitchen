...
class TreeHash implements ChunkHashInterface
{
    ...
    public static function fromChecksums(
        ...
        $treeHash->checksums = $inBinaryForm
            ? $checksums
            : array_map('Aws\Common\Hash\HashUtils::hexToBin', $checksums);
    ...
    public function addData($data)
    {
        ...
        if ($this->hash) {
            throw new LogicException(
                'You may not add more data to a finalized tree hash.'
            );
        }
        ...
        if (strlen($data) > Size::MB) {
            throw new InvalidArgumentException(
                'The chunk of data added is too large for tree hashing.'
            );
        }
    ...
    public function addChecksum($checksum, $inBinaryForm = false)
    {
        ...
        if ($this->hash) {
            throw new LogicException(
                'You may not add more checksums to a finalized tree hash.'
            );
        }

        // Convert the checksum to binary form if necessary
        $this->checksums[] = $inBinaryForm
            ? $checksum
            : HashUtils::hexToBin($checksum);
