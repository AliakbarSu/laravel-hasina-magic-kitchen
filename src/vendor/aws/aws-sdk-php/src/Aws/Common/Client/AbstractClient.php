...
abstract class AbstractClient extends Client implements AwsClientInterface
{
    ...
    {
        return array_merge(Client::getAllEvents(), [
    ...
    public function __construct(
        ...
        $dispatcher->addSubscriber(
            new SignatureListener($credentials, $signature)
        );
    ...
    public function setCredentials(CredentialsInterface $credentials)
    {
        ...
        $this->dispatch('client.credentials_changed', [
    ...
    public function setRegion($region)
    {
        ...
        if (
            !$global ||
            $this->serviceDescription->getData('namespace') === 'S3'
        ) {
            $endpoint = call_user_func($provider, [
            ...
            $this->dispatch('client.region_changed', [
                'region' => $region,
    ...
    public function waitUntil($waiter, array $input = [])
    {
    ...
    public function getWaiter($waiter, array $input = [])
    {
        return $this->getWaiterFactory()
            ->build($waiter)
            ->setClient($this)
            ->setConfig($input);
    }
    ...
    public function getWaiterFactory()
    {
        if (!$this->waiterFactory) {
            ...
            $this->waiterFactory = new CompositeWaiterFactory([
                new WaiterClassFactory(
                    substr($clientClass, 0, strrpos($clientClass, '\\')) .
                        '\\Waiter'
                ),
            ]);
            if ($this->getDescription()) {
                ...
                $this->waiterFactory->addFactory(
                    new WaiterConfigFactory($waiterConfig)
                );
            }
        }
