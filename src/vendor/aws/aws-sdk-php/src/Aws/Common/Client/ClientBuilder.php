...
class ClientBuilder
{
     ...
     */
    protected static $commonConfigDefaults = ['scheme' => 'https'];

    /**
     * @var array Default client requirements
     */
    protected static $commonConfigRequirements = [Options::SERVICE_DESCRIPTION];
    ...
    public function __construct($namespace = null)
    {
        ...
        if ($this->clientNamespace) {
            $this->serviceName = substr(
                $this->clientNamespace,
                strrpos($this->clientNamespace, '\\') + 1
            );
            $this->clientClass =
                $this->clientNamespace . '\\' . $this->serviceName . 'Client';
        }
    }
    ...
    public function build()
    {
        ...
        if ($config[Options::VERSION] === 'latest') {
            $config[Options::VERSION] = constant(
                "{$this->clientClass}::LATEST_API_VERSION"
            );
        }
        ...
        if ($backoff === null) {
            $retries = isset($config[Options::BACKOFF_RETRIES])
                ? $config[Options::BACKOFF_RETRIES]
                : 3;
        ...
        $client->setResourceIteratorFactory(
            new AwsResourceIteratorFactory(
                $this->iteratorsConfig,
                new ResourceIteratorClassFactory(
                    $this->clientNamespace . '\\Iterator'
                )
            )
        );
        ...
        if ($config->get(Options::VALIDATION) === false) {
            $params = $config->get('command.params') ?: [];
     ...
     * @throws InvalidArgumentException
     */
    protected function addBackoffLogger(
        BackoffPlugin $plugin,
        Collection $config
    ) {
        ...
        if ($logger = $config->get(Options::BACKOFF_LOGGER)) {
            ...
            if ($logger === 'debug') {
                ...
                throw new InvalidArgumentException(
                    Options::BACKOFF_LOGGER .
                        ' must be set to `debug` or an instance of ' .
    ...
    protected function getSignature(
        ...
        $signature =
            ...
            $description->getData('signatureVersion');
        ...
        if ($signature instanceof EndpointSignatureInterface) {
            ...
            $signature->setServiceName(
                $config->get(Options::SIGNATURE_SERVICE) ?:
                $description->getData('signingName') ?:
                $description->getData('endpointPrefix')
            );

            // Determine the region to use when signing requests
            $signature->setRegionName(
                $config->get(Options::SIGNATURE_REGION) ?:
                $config->get(Options::REGION)
            );
        }
    ...
    private function handleEndpoint(Collection $config)
    {
        ...
        $endpoint = call_user_func($config['endpoint_provider'], [
        ...
        if (
            !$config->hasKey(Options::SIGNATURE) &&
