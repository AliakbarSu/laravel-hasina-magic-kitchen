...
class ConfigResourceWaiter extends AbstractResourceWaiter
{
    ...
    public function setConfig(array $config)
    {
        ...
        if (!isset($config[self::INTERVAL])) {
            $config[self::INTERVAL] = $this->waiterConfig->get(
                WaiterConfig::INTERVAL
            );
        }

        if (!isset($config[self::MAX_ATTEMPTS])) {
            $config[self::MAX_ATTEMPTS] = $this->waiterConfig->get(
                WaiterConfig::MAX_ATTEMPTS
            );
        }
    ...
    protected function doWait()
    {
        ...
        $operation = $this->client->getCommand(
            $this->waiterConfig->get(WaiterConfig::OPERATION),
            $params
        );
        ...
        try {
            ...
            throw new InvalidArgumentException(
                $this->waiterConfig->get(WaiterConfig::WAITER_NAME) .
                    ' waiter validation failed:  ' .
                    $e->getMessage(),
                $e->getCode(),
                $e
            );
            ...
            foreach (
                (array) $this->waiterConfig->get(WaiterConfig::IGNORE_ERRORS)
                as $ignore
            ) {
    ...
    protected function checkResult(Model $result)
    {
        ...
        if ($this->waiterConfig->get(WaiterConfig::FAILURE_TYPE) == 'output') {
            $failureValue = $this->waiterConfig->get(
                WaiterConfig::FAILURE_VALUE
            );
            if ($failureValue) {
                ...
                if ($this->checkPath($result, $key, $failureValue, false)) {
                    ...
                    throw new RuntimeException(
                        'A resource entered into an invalid state of "' .
                            implode(', ', $triggered) .
                            '" while waiting with the "' .
                            $this->waiterConfig->get(
                                WaiterConfig::WAITER_NAME
                            ) .
                            '" waiter.'
                    );
                }
