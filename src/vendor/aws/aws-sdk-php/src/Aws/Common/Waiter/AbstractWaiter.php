 ...
 */
abstract class AbstractWaiter extends AbstractHasDispatcher implements
    WaiterInterface
{
    ...
    {
        return isset($this->config[self::MAX_ATTEMPTS])
            ? $this->config[self::MAX_ATTEMPTS]
            : 10;
    }
    ...
    {
        return isset($this->config[self::INTERVAL])
            ? $this->config[self::INTERVAL]
            : 0;
    }
    ...
    public function wait()
    {
        ...
        do {
            $this->dispatch('waiter.before_attempt', [
                'waiter' => $this,
                'config' => $this->config,
            ]);
            ...
            if (++$this->attempts >= $this->getMaxAttempts()) {
                throw new RuntimeException(
                    'Wait method never resolved to true after ' .
                        $this->attempts .
                        ' attempts'
                );
            }

            $this->dispatch('waiter.before_wait', [
                'waiter' => $this,
                'config' => $this->config,
            ]);
