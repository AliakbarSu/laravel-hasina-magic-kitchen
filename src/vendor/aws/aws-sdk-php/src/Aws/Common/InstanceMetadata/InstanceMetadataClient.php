...
class InstanceMetadataClient extends AbstractClient
{
    ...
    public function getInstanceProfileCredentials()
    {
        try {
            ...
            $result = $this->get(
                "meta-data/iam/security-credentials/{$credentials}"
            )
                ->send()
                ->json();
        } catch (\Exception $e) {
            $message = sprintf(
                'Error retrieving credentials from the instance profile metadata server. When you are' .
                    ...
                    ' Aws\\Common\\Credentials\\CredentialsInterface object. (%s)',
                $e->getMessage()
            );
            throw new InstanceProfileCredentialsException(
                $message,
                $e->getCode()
            );
        }
        ...
        if ($result['Code'] !== 'Success') {
            $e = new InstanceProfileCredentialsException(
                'Unexpected response code: ' . $result['Code']
            );
