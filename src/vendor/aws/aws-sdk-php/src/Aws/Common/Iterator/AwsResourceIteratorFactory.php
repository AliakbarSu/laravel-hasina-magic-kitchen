...
class AwsResourceIteratorFactory implements ResourceIteratorFactoryInterface
{
     ...
     */
    protected static $defaultIteratorConfig = [
        'input_token' => null,
        'output_token' => null,
        'limit_key' => null,
        'result_key' => null,
    ...
    public function __construct(
        array $config,
        ResourceIteratorFactoryInterface $primaryIteratorFactory = null
    ) {
    ...
    public function build(CommandInterface $command, array $options = [])
    {
        ...
        $options += $commandSupported ? $this->config[$commandName] : [];

        // Instantiate the iterator using the primary factory (if one was provided)
        if (
            $this->primaryIteratorFactory &&
            $this->primaryIteratorFactory->canBuild($command)
        ) {
            $iterator = $this->primaryIteratorFactory->build(
                $command,
                $options
            );
        } elseif (!$commandSupported) {
            throw new InvalidArgumentException(
                "Iterator was not found for {$commandName}."
            );
        } else {
