...
class AwsResourceIterator extends ResourceIterator
{
    ...
    protected function handleResults(Model $result)
    {
        ...
        if ($resultKey = $this->get('result_key')) {
            $results = $this->getValueFromResult($result, $resultKey) ?: [];
        }
    ...
    protected function applyNextToken()
    {
        ...
        if ($tokenParam = $this->get('input_token')) {
            ...
            if (is_array($tokenParam)) {
                if (
                    is_array($this->nextToken) &&
                    count($tokenParam) === count($this->nextToken)
                ) {
                    foreach (
                        array_combine($tokenParam, $this->nextToken)
                        as $param => $token
                    ) {
                    ...
                    throw new RuntimeException(
                        'The definition of the iterator\'s token parameter and the actual token ' .
            ...
            if ($tokenKey = $this->get('output_token')) {
                ...
                if (is_array($tokenKey)) {
                    ...
                    foreach ($tokenKey as $key) {
                        $this->nextToken[] = $this->getValueFromResult(
                            $result,
                            $key
                        );
                    }
