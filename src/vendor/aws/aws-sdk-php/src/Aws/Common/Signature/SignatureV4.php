 ...
 */
class SignatureV4 extends AbstractSignature implements
    EndpointSignatureInterface
{
    ...
    public function signRequest(
        ...
        if ($request->hasHeader('x-amz-date') || !$request->hasHeader('Date')) {
            ...
            $request->setHeader(
                'Date',
                gmdate(DateFormat::RFC1123, $timestamp)
            );
        }
     ...
     */
    public static function convertPostToGet(
        EntityEnclosingRequestInterface $request
    ) {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException(
                'Expected a POST request but ' .
                    'received a ' .
                    $request->getMethod() .
                    ' request.'
            );
        ...
        $cloned = RequestFactory::getInstance()->cloneRequestWithMethod(
            $request,
            'GET'
        );
    ...
    private function createPresignedRequest(
        ...
        if (
            ...
            $sr = RequestFactory::getInstance()->cloneRequestWithMethod(
                $request,
                'GET'
            );
            // Move POST fields to the query if they are present
    ...
    {
        $cacheKey =
            $shortDate . '_' . $region . '_' . $service . '_' . $secretKey;
        ...
        if (!isset($this->hashCache[$cacheKey])) {
            ...
            $this->hashCache[$cacheKey] = hash_hmac(
                'sha256',
                'aws4_request',
                $serviceKey,
                true
