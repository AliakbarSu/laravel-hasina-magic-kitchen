...
class AwsQueryVisitor extends AbstractRequestVisitor
{
     ...
     */
    protected function customResolver(
        $value,
        Parameter $param,
        array &$query,
        $prefix = ''
    ...
    protected function resolveObject(
        ...
        foreach ($value as $name => $v) {
            if ($subParam = $param->getProperty($name)) {
                ...
                $valueName = isset($data['valueName'])
                    ? $data['valueName']
                    : 'value';
                $query[
                    "{$prefix}.{$additionalPropertyCount}.{$keyName}"
                ] = $name;
                $newPrefix = "{$prefix}.{$additionalPropertyCount}.{$valueName}";
                if (is_array($v)) {
                    $this->customResolver(
                        $v,
                        $param->getAdditionalProperties(),
                        $query,
                        $newPrefix
                    );
                } else {
