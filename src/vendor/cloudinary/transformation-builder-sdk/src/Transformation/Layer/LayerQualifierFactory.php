...
class LayerQualifierFactory
{
    ...
    public static function fromParams(
        $layerQualifiers,
        $layerStackPosition = LayerStackPosition::OVERLAY
    ) {
        return self::handleQualifierValue($layerQualifiers)->setStackPosition(
            $layerStackPosition
        );
    }
    ...
    protected static function handleQualifierValue($layerQualifiers)
    {
        ...
        if (is_array($layerQualifiers)) {
            ...
            if (!empty($text) || $resourceType === 'text') {
                ...
                return (new TextSourceQualifier(
                    $text,
                    $textStyle
                ))->styleFromPublicId($publicId);
            }

            if ($publicId === null) {
                throw new InvalidArgumentException(
                    "Must supply public_id for $resourceType layer"
                );
            }
            ...
            if ($resourceType === 'subtitles') {
                return new SubtitlesSourceQualifier(
                    $publicId,
                    $layerQualifiers
                );
            }
            ...
            $layerQualifiers = ArrayUtils::implodeQualifierValues(
                ...$components
            );
