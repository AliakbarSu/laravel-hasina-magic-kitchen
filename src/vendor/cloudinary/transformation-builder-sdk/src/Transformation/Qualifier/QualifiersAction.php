...
class QualifiersAction extends BaseAction
{
    ...
    {
        $this->qualifiers = ArrayUtils::mergeNonEmpty(
            $this->qualifiers,
            ...$qualifiers
        );
    ...
    {
        $options =
            is_array($transformationAction) ||
            $this->isTransformationComponent($transformationAction)
    ...
    protected function generateTransformationString($options)
    {
        ...
        if (is_array($options) && !ArrayUtils::isAssoc($options)) {
            return ArrayUtils::implodeUrl(
                array_map([$this, 'generateTransformationAction'], $options)
            );
        }
        ...
        $nestedTransformations = ArrayUtils::build(
            ArrayUtils::pop($options, 'transformation')
        );
        if (
            count(
                array_filter($nestedTransformations, [
                    $this,
                    'isTransformation',
                ])
            ) > 0
        ) {
            $baseTransformations = array_map(
                [$this, 'generateTransformationAction'],
                $nestedTransformations
            );
        } else {
            // array of strings (named transformations)
        ...
        $transformation = ArrayUtils::implodeFiltered(',', [
            $if,
            $variables,
            $qualifiersAction,
            $rawTransformation,
    ...
    protected function collectQualifiers($options)
    {
        ...
        $qualifiers = [];
        foreach (
            array_intersect_key($options, self::QUALIFIERS)
            as $qualifierName => $qualifierValue
        ) {
            ArrayUtils::appendNonEmpty(
                $qualifiers,
                $this->buildQualifier($qualifierName, $qualifierValue)
            );
        }
    ...
    protected function buildQualifier($qualifierName, $qualifierValue)
    {
        if (array_key_exists($qualifierName, self::SIMPLE_QUALIFIERS)) {
            $qualifierBuilderName = StringUtils::snakeCaseToCamelCase(
                ArrayUtils::get(self::SIMPLE_QUALIFIERS, $qualifierName) ?:
                $qualifierName
            );
        ...
        switch ($qualifierName) {
            ...
            case 'flags':
                return Qualifier::flag(
                    implode('.', ArrayUtils::build($qualifierValue))
                );
            ...
            case 'overlay':
                return LayerQualifierFactory::fromParams(
                    $qualifierValue,
                    LayerStackPosition::OVERLAY
                );
            case 'transformation':
                return Qualifier::namedTransformation(
                    implode('.', ArrayUtils::build($qualifierValue))
                );
            case 'underlay':
                return LayerQualifierFactory::fromParams(
    ...
    private function isTransformation($candidate)
    {
        return is_array($candidate) ||
            $this->isTransformationComponent($candidate);
    }
}
