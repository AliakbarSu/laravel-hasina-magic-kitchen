...
class Preview extends EffectAction
{
    ...
    {
        $this->getMainQualifier()->add(
            ClassUtils::verifyInstance($duration, PreviewDuration::class)
        );
    ...
    public function maximumSegments($maximumSegments)
    {
        if ($maximumSegments) {
            $this->getMainQualifier()->add(
                new GenericNamedArgument('max_seg', $maximumSegments)
            );
        }
    ...
    public function minimumSegmentDuration($minimumSegmentDuration)
    {
        if ($minimumSegmentDuration) {
            $this->getMainQualifier()->add(
                new GenericNamedArgument('min_seg_dur', $minimumSegmentDuration)
            );
        }
