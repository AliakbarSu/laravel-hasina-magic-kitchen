...
class RemoveBackground extends EffectAction
{
    ...
    {
        parent::__construct(
            new EffectQualifier(PixelEffect::REMOVE_BACKGROUND)
        );

        $this->getMainQualifier()
            ->getValue()
            ->setArgumentOrder([0, self::SCREEN, self::COLOR_TO_REMOVE]);
    ...
    public function screen($screen = true)
    {
        if ($screen) {
            $this->getMainQualifier()
                ->getValue()
                ->setSimpleValue(self::SCREEN, self::SCREEN);
        }
    ...
    public function colorToRemove($colorToRemove)
    {
        if ($colorToRemove) {
            ...
            $colorToRemove = StringUtils::truncatePrefix(
