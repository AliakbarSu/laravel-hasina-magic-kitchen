...
abstract class BaseAction extends BaseComponent
{
    ...
    public function setGenericAction($action)
    {
        if (StringUtils::contains($action, '/')) {
            throw new InvalidArgumentException(
                'A single generic action must be supplied'
            );
        }
    ...
    public function jsonSerialize()
    {
        ...
        ArrayUtils::addNonEmpty(
            $jsonArray,
            'qualifiers',
            self::jsonSerializeQualifiers($this->qualifiers)
        );
        ArrayUtils::addNonEmpty($jsonArray, 'generic', $this->genericAction);
        ArrayUtils::addNonEmpty(
            $jsonArray,
            'flags',
            self::jsonSerializeQualifiers($this->flags)
        );
    ...
    public function getStringQualifiers()
    {
        ...
        foreach ($this->qualifiers as $qualifier) {
            $flatQualifiers = ArrayUtils::mergeNonEmpty(
                $flatQualifiers,
                $qualifier->getStringQualifiers()
            );
        }

        return array_merge(
            $flatQualifiers,
            [self::serializeFlags($this->flags)],
            [$this->genericAction]
        );
    }
    ...
    {
        return ArrayUtils::implodeActionQualifiers(
            ...$this->getStringQualifiers()
        );
    }
    ...
    {
        $result = array_map(static function ($qualifier) {
