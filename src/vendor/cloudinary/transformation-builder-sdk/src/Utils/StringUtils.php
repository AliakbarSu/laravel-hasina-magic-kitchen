...
class StringUtils
{
    ...
    {
        return strtolower(
            preg_replace('/(?<!^)[A-Z]/', $separator . '$0', $input)
        );
    }
    ...
    public static function toAcronym($input, $exclusions = [], $delimiter = '_')
    {
        ...
        foreach (
            ArrayUtils::safeFilter(
                ArrayUtils::blacklist(explode($delimiter, $input), $exclusions)
            )
            as $word
        ) {
            $acronym .= $word[0];
    ...
    public static function truncateMiddle(
        ...
        return sprintf(
            '%s%s%s',
            substr($string, 0, $numLeftChars),
            $glue,
            substr($string, 0 - $numRightChars)
        );
    }
