...
class ArrayUtils
{
    ...
    public static function implodeAssoc($array, $outer = '', $inner = null)
    {
        ...
        if (self::isAssoc($array)) {
            // implode key-value pairs using inner
            $array = self::mapAssoc(static function ($k, $v) use ($inner) {
    ...
    public static function safeImplodeAssoc(
        ...
        if (self::isAssoc($array)) {
            // implode key-value pairs using inner
            $array = self::mapAssoc(static function ($k, $v) use (
                $inner,
                $innerIsSafe
            ) {
                if ($innerIsSafe) {
    ...
    public static function implodeFiltered(
        ...
        return self::safeImplode(
            $glue,
            self::safeFilter($pieces, $filterCallback, $flag)
        );
    }
    ...
    {
        array_walk($pieces, static function (&$value) {
    ...
    public static function escapedImplode($glue, array $pieces)
    {
        return implode(
            $glue,
            array_map(static function ($value) use ($glue) {
    ...
    public static function sortByArray($array, $orderArray = [])
    {
        ...
        return array_replace(
            array_intersect_key(array_flip($orderArray), $array),
            $array
        );
    }
    ...
    public static function get($array, $key, $default = null)
    {
        if (is_array($key)) {
            ...
            foreach ($key as $currKey) {
                if (
                    !is_array($currLevel) ||
                    !array_key_exists($currKey, $currLevel)
                ) {
                    //not found
                    return $default;
                }
    ...
    public static function addNonEmptyFromOther(
        ...
        return self::addNonEmpty(
            $resultingArr,
            $key,
            self::get($sourceArray, $key, $defaultValue)
        );
    }
