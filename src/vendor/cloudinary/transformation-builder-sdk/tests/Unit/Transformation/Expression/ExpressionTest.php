...
final class ExpressionTest extends TestCase
{
    ...
    public function testLogicalExpression()
    {
        ...
        $le = new ExpressionComponent(
            new ExpressionComponent($re, LogicalOperator::andOperator(), $re),
    ...
    public function testExpressionBuilder()
    {
        ...
        self::assertEquals(
            '-1_add_iar_add_tar_add_pc_add_cp',
            (string) $addExpr
        );
        ...
        self::assertEquals(
            '5_add_ils_add_px_add_py_add_ctx_add_ar_add_tags',
            (string) $addExpr
        );
    }
    ...
    public function testVariableExpression()
    {
        ...
        $moduloExpr = UVar::uVar('myVar')
            ->modulo()
            ->generic('w');

        self::assertEquals('$myVar_mod_w', (string) $moduloExpr);

        $expr1 = UVar::uVar('myVar')
            ->equal()
            ->height()
            ->and_()
            ->width()
        ...
        $expr3 = UVar::uVar('myVar')
            ->greaterThanOrEqual()
            ->numeric(9)
            ->add()
            ->assetReference('public_id');

        self::assertEquals('$myVar_gte_9_add_ref:!public_id!', (string) $expr3);

        $expr4 = UVal::uVal('myValue')
            ->equal()
            ->string('d')
            ->notEqual()
            ->string('s');
        ...
        self::assertEquals(
            '$myVar_mod_w_add_17_add_w',
            (string) $moduloAddExpr
        );

        $inExpr = PVar::tags()
            ->in()
            ->stringArray(['apple', 'orange', 'kiwi'])
            ->notIn()
            ->stringArray(['chery']);

        self::assertEquals(
            'tags_in_!apple:orange:kiwi!_nin_!chery!',
            (string) $inExpr
        );

        $expressionOperator = new ExpressionOperator(
            new ExpressionComponent(PVar::initialAspectRatio())
        );

        self::assertEquals(
            'iar_$vName',
            (string) $expressionOperator->uVar('vName')
        );
    }

    public function testExpressionInParameter()
    {
        $addExpr = UVar::uVar('someVal')
            ->add()
            ->width();
    ...
    {
        $expr =
            'width = 0 && height != 0 || aspect_ratio < 0 && page_count > 0 and face_count <= 0 and width >= 0';

        $rawExpr = Expression::expression($expr);

        $expectedExpr =
            'w_eq_0_and_h_ne_0_or_ar_lt_0_and_pc_gt_0_and_fc_lte_0_and_w_gte_0';

        self::assertEquals($expectedExpr, (string) $rawExpr);

        self::assertEquals(
            $expectedExpr . '_gt_997',
            (string) $rawExpr->greaterThan()->numeric(997)
        );
    }
    ...
    public static function normalizationDataProvider()
    {
        return [
            ...
            'double ampersand replaced with and operator' => [
                'foo && bar',
                'foo_and_bar',
            ],
            ...
            'width recognized as variable and replaced with w' => [
                'width',
                'w',
            ],
            ...
            '$width recognized as user variable and not affected' => [
                '$width',
                '$width',
            ],
            ...
            '$mywidth recognized as user variable and not affected' => [
                '$mywidth',
                '$mywidth',
            ],
            ...
            '$_width recognized as user variable and not affected' => [
                '$_width',
                '$_width',
            ],
            '$__width recognized as user variable and not affected' => [
                '$__width',
                '$_width',
            ],
            '$$width recognized as user variable and not affected' => [
