...
final class VideoOverlayTest extends TestCase
{
    public function testVideoOverlay()
    {
        ...
        $t->resize(
            Crop::crop(100, 200)->gravity(Gravity::compass(Compass::south()))
        );
        ...
        self::assertEquals($expected, (string) $videoOverlayConstructed);
        self::assertInstanceOf(
            ImageSource::class,
            $videoOverlayConstructed->jsonSerialize()['source']
        );
        self::assertInstanceOf(
            CompassPosition::class,
            $videoOverlayConstructed->jsonSerialize()['position']
        );

        $videoOverlayMethods = (new VideoOverlay($il))
            ->position($p)
            ->timeline($tlp);
        self::assertEquals($expected, (string) $videoOverlayMethods);
        self::assertInstanceOf(
            ImageSource::class,
            $videoOverlayMethods->jsonSerialize()['source']
        );
        self::assertInstanceOf(
            CompassPosition::class,
            $videoOverlayMethods->jsonSerialize()['position']
        );
    }
    ...
    public function testVideoConcatenate()
    {
        ...
        self::assertEquals(
            ...
            (string) (new VideoTransformation())->videoEdit(
