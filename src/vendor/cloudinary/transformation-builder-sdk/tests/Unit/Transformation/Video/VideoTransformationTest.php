 ...
 */
final class VideoTransformationTest extends TransformationTestCase
{
    public function testVideoTransformation()
    {
        ...
        $t->resize(
            Scale::scale(new Width(100), 200)->aspectRatio(
                AspectRatio::ignoreInitialAspectRatio()
            )
        )
            ->resize(Pad::limitPad(50))
            ->rotate(17);

        $t_expected =
            'c_scale,fl_ignore_aspect_ratio,h_200,w_100/c_lpad,w_50/a_17';
        self::assertEquals($t_expected, (string) $t);

        $t->trim(Timeline::position('auto', '90%', 10.1));

        $t2_expected = $t_expected . '/du_10.1,eo_90p,so_auto';
        self::assertEquals($t2_expected, (string) $t);

        $t->roundCorners(17);

        $t3_expected = $t2_expected . '/r_17';
        self::assertEquals($t3_expected, (string) $t);

        $t->rotate(17);

        $t4_expected = $t3_expected . '/a_17';
        self::assertEquals($t4_expected, (string) $t);

        $t->overlay('dog');

        $t5_expected = $t4_expected . '/l_video:dog/fl_layer_apply';
        self::assertEquals($t5_expected, (string) $t);
    }
    ...
    public function testVideoTransformationVideoEdit()
    {
        ...
        self::assertEquals('du_10.1,eo_90p,so_auto', (string) $t);
    }
    ...
    public function testVideoTransformationLayers()
    {
        ...
        self::assertEquals(
            ...
            (string) (new VideoTransformation())->overlay(
                new SubtitlesSource('sample_sub_en.srt')
            )
        );
        ...
        self::assertEquals(
            ...
            (string) (new VideoTransformation())->adjust(
    ...
    public function testVideoTransformationFps()
    {
        self::assertEquals(
            'fps_25',
            (string) (new VideoTransformation())->fps(25)
        );

        self::assertEquals(
            'fps_25-30',
            (string) (new VideoTransformation())->transcode(
                Transcode::fpsRange(25, 30)
            )
        );
        ...
        self::assertEquals(
            'fps_25',
            (string) (new VideoTransformation())->fps(new Fps(25))
        );
    }
    ...
    public function testVideoTransformationKeyframeInterval()
    {
        self::assertEquals(
            'ki_0.15',
            (string) (new VideoTransformation())->keyframeInterval(0.15)
        );
    }
    ...
    public function testVideoTransformationStreamingProfile()
    {
        self::assertEquals(
            'sp_full_hd',
            (string) (new VideoTransformation())->streamingProfile(
    ...
    public function testVideoTransformationBitRate()
    {
        self::assertEquals(
            'br_120000',
            (string) (new VideoTransformation())->bitRate(120000)
        );

        self::assertEquals(
            'br_250k',
            (string) (new VideoTransformation())->bitRate('250k')
        );

        self::assertEquals(
            'br_2m',
            (string) (new VideoTransformation())->bitRate('2m')
        );
    }
    ...
    public function testVideoTransformationVariousParams()
    {
        self::assertStrEquals(
            'vs_2',
            (new VideoTransformation())->videoSampling(2)
        );
    }
    ...
    public function testVideoTransformationFlags()
    {
        self::assertEquals(
            ...
            (string) (new VideoTransformation())->streamingAttachment(
                'my_streaming_video.mp4'
            )
        );

        self::assertEquals(
            'fl_hlsv3',
            (string) (new Transformation())->hlsv3()
        );
    }

    public function testVideoTransformationRange()
    {
        self::assertEquals('1.2,3.4', (string) new Range([1.2, 3.4]));
    }
