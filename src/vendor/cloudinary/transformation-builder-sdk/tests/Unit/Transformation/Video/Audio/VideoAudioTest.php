 ...
 */
final class VideoAudioTest extends TransformationTestCase
{
    ...
    public function testAudioCodec()
    {
        ...
        self::assertEquals('ac_aac', (string) $aac);

        $acNone = AudioCodec::none();

        self::assertEquals('ac_none', (string) $acNone);
    }
    ...
    public function testVideoFPS()
    {
        ...
        foreach ($fpsTestValues as $value) {
            self::assertEquals($value[1], (string) Fps::fromParams($value[0]));
        }

        self::assertEquals('fps_22-24', (string) (new Fps())->min(22)->max(24));
    }
     ...
     * @param array  $args
     */
    public function testExceptionsKeyframeInterval(
        $exceptionClass,
        $exceptionMessage,
        $args
    ) {
