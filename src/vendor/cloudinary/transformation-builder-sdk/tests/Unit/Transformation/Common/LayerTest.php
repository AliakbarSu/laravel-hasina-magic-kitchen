...
final class LayerTest extends TransformationTestCase
{
    public function testImageLayer()
    {
        ...
        $t_expected =
            'c_thumb,g_auto:adv_eyes,h_200,w_100/e_hue:99/e_replace_color:pink:50:cyan/bo_solid';

        $publicId = 'some/asset/in/path/test';

        $imageLayer = (new ImageSource($publicId))
            ->transformation($t)
            ->addTransformation($t2);

        $t_t2_expected = "l_some:asset:in:path:test/$t_expected/fl_layer_apply";
        self::assertEquals($t_t2_expected, (string) $imageLayer);
    ...
    public function testImageLayerWithPosition()
    {
        ...
        $t->resize(
            ...
            ->adjust(Adjust::hue(99))
            ->adjust(
                Adjust::replaceColor(ColorValue::pink(), 50, ColorValue::cyan())
            );
    ...
    public function testTextLayer()
    {
        ...
        $t->adjust(Adjust::hue(99))->adjust(
            Adjust::replaceColor(Color::PINK, 50, Color::CYAN)
        );
    ...
    public function testMediaOverlay()
    {
        self::assertStrEquals(
            ...
            (new MediaOverlay('media'))
                ->position(Position::south())
                ->blendMode(BlendMode::mask())
                ->timeline(Timeline::position()->startOffset(5))
        );
    }
    ...
    public function testTextLayerParameter()
    {
        ...
        $style = (new TextStyle(FontFamily::ARIAL, 14))
            ...
            ->fontWeight(FontWeight::DEMIBOLD);
        $styleExpected =
            'Arial_14_demibold_antialias_fast_hinting_slight_letter_spacing_17';
