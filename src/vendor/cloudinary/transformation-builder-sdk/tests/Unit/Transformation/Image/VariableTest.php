...
final class VariableTest extends TestCase
{
    public function testVariable()
    {
        ...
        self::assertEquals(
            '$var_100_to_f',
            (string) Variable::set('var', 100)->asFloat()
        );
        self::assertEquals(
            '$var_100_to_i',
            (string) Variable::set('var', 100)->asInteger()
        );
        self::assertEquals('$var_!100!', (string) Variable::set('var', '100'));
        self::assertEquals(
            '$var_iw',
            (string) Variable::set('var', PVar::initialWidth())
        );
        self::assertEquals(
            '$var_!apple:orange:kiwi!',
            (string) Variable::set('var', ['apple', 'orange', 'kiwi'])
        );
        self::assertEquals(
            '$var_ref:!asset!',
            (string) Variable::setAssetReference('var', 'asset')
        );
        self::assertEquals(
            '$var_ctx:!ctx_key!',
            (string) Variable::setFromContext('var', 'ctx_key')
        );
        self::assertEquals(
            '$var_md:!md_key!',
            (string) Variable::setFromMetadata('var', 'md_key')
        );
    }
