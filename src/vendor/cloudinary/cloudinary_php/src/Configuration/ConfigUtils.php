...
class ConfigUtils
{
    ...
    {
        return Utils::tryParseUrl(
            self::normalizeCloudinaryUrl($cloudinaryUrl),
            [self::CLOUDINARY_URL_SCHEME]
        )
            ? true
            : false;
    }
    ...
    public static function parseCloudinaryUrl($cloudinaryUrl)
    {
        ...
        $qParams = Utils::tryParseValues(
            StringUtils::parseQueryString($uri->getQuery())
        );
        ...
        $config = array_merge_recursive($qParams, [
            CloudConfig::CONFIG_NAME => $cloud,
        ]);
    ...
    public static function normalizeCloudinaryUrl($cloudinaryUrl)
    {
        ...
        return StringUtils::truncatePrefix(
            $cloudinaryUrl,
            Configuration::CLOUDINARY_URL_ENV_VAR . '='
        );
    }
    ...
    public static function buildCloudinaryUrl($config)
    {
        ...
        if (!empty($config[CloudConfig::CONFIG_NAME])) {
            $res .=
                "{$config[CloudConfig::CONFIG_NAME][CloudConfig::API_KEY]}:" .
                "{$config[CloudConfig::CONFIG_NAME][CloudConfig::API_SECRET]}@";
        }

        $res .= ArrayUtils::get($config, [
        ...
        $res = ArrayUtils::implodeFiltered('/', [
            $res,
            ArrayUtils::get($config, [
                UrlConfig::CONFIG_NAME,
                UrlConfig::SECURE_CNAME,
            ]),
        ]);
