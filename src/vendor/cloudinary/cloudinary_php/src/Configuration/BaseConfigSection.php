...
abstract class BaseConfigSection implements ConfigurableInterface
{
    ...
    public function __get($property)
    {
        if (!property_exists($this, $property)) {
            trigger_error(
                'Undefined property: ' . static::class . '::$' . $property,
                E_USER_NOTICE
            );
        ...
        if (!isset($this->{$property})) {
            $defaultConstName =
                'DEFAULT_' .
                strtoupper(StringUtils::camelCaseToSnakeCase($property));
    ...
    {
        return ArrayUtils::get(
            $this->explicitlySetKeys,
            StringUtils::snakeCaseToCamelCase($name),
            false
        );
    }
    ...
    {
        $validKeys = self::importableKeys(
            self::exportableKeys($includeSensitive)
        );
        ...
        foreach ($validParams as $name => $value) {
            $propertyName = StringUtils::snakeCaseToCamelCase(
                ArrayUtils::get(static::$aliases, $name, $name)
            );
    ...
    protected static function exportableKeys($includeSensitive = true)
    {
        ...
        if (!$includeSensitive) {
            $blacklisted = array_merge(
                $blacklisted,
                static::$sensitiveDataKeys
            );
        }
     ...
     * @return static
     */
    public static function fromCloudinaryUrl(
        $cloudinaryUrl,
    ...
    public function toString($excludedKeys = [])
    {
        ...
        $sectionJson[static::CONFIG_NAME] = ArrayUtils::blacklist(
            $sectionJson[static::CONFIG_NAME],
            $excludedKeys
        );
    ...
    public function jsonSerialize(
        ...
        foreach (self::exportableKeys($includeSensitive) as $key) {
            ...
            if (
                property_exists(static::class, $propertyName) &&
