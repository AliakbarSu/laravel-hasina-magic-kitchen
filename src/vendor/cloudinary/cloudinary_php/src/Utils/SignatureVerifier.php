...
class SignatureVerifier
{
    ...
    public static function verifyNotificationSignature(
        ...
        self::validateParams(
            $paramsArray,
            self::$NOTIFICATION_VALIDATOR_ALLOWED_TYPES
        );
     ...
     * @throws InvalidArgumentException in case a mandatory parameter is empty or of wrong type
     */
    public static function verifyApiResponseSignature(
        $publicId,
        $version,
        $signature
    ) {
        ...
        self::validateParams(
            $paramsArray,
            self::$API_RESPONSE_VALIDATOR_ALLOWED_TYPES
        );
        ...
        foreach ($allowedTypes as $param => $types) {
            if (empty($params[$param])) {
                throw new InvalidArgumentException(
                    "$param parameter cannot be empty"
                );
            }

            if (!self::paramValidator($params[$param], $types)) {
                throw new InvalidArgumentException(
                    "$param must be one of the following types: $types"
                );
            }
        }
    }
