...
class Analytics
{
    ...
    {
        static::$techVersion = join(
            '.',
            array_slice(explode('.', $techVersion), 0, 2)
        );
    }
    ...
    protected static function encodeVersion($version)
    {
        ...
        $paddedParts = array_map(static function ($part) {
            return str_pad((int) $part, 2, '0', STR_PAD_LEFT); // this also zeros non-numeric values
        }, $parts);

        $number = (int) implode(array_reverse($paddedParts));
        $paddedBinary = self::intToPaddedBin(
            $number,
            count($parts) * self::BINARY_PAD_SIZE
        );

        if (strlen($paddedBinary) % self::BINARY_PAD_SIZE !== 0) {
            throw new OutOfRangeException(
                'Version must be smaller than 43.21.26'
            );
        }

        $encodedChars = array_map(static function ($part) {
    ...
    protected static function getKey($binaryValue)
    {
        if (empty(self::$charCodes)) {
            ...
            for ($i = 0, $len = strlen(self::CHARS); $i < $len; $i++) {
                self::$charCodes[
                    self::intToPaddedBin($i, self::BINARY_PAD_SIZE)
                ] = self::CHARS[$i];
            }
        }
