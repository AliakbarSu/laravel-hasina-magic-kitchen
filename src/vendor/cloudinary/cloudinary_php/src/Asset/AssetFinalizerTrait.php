...
trait AssetFinalizerTrait
{
        ...
        if ($this->urlConfig->secure) {
            ...
            if ($secureCdnSubDomain) {
                $hostName = str_replace(
                    ...
                    self::buildHostName(
                        null,
                        self::domainShard($this->asset->publicId())
                    ),
                    $hostName
                );
            }
            ...
            if ($this->urlConfig->cname) {
                $hostName = self::buildHostName(
                    null,
                    null,
                    $this->urlConfig->cdnSubdomain
                        ? 'a' . self::domainShard($this->asset->publicId())
                        : null,
                    $this->urlConfig->cname
                );
            } else {
                $hostName = self::buildHostName(
                    $this->urlConfig->privateCdn
    ...
    protected function finalizeSimpleSignature()
    {
        ...
        return Utils::formatSimpleSignature(
            $signature,
            $this->urlConfig->longUrlSignature
                ? Utils::LONG_URL_SIGNATURE_LENGTH
                : Utils::SHORT_URL_SIGNATURE_LENGTH
        );
    }
    ...
    protected function getSignatureAlgorithm()
    {
        ...
        if (
            ArrayUtils::inArrayI(
                $this->cloud->signatureAlgorithm,
                Utils::SUPPORTED_SIGNATURE_ALGORITHMS
            )
        ) {
    ...
    protected function finalizeUrlWithAnalytics($urlParts)
    {
        ...
        if (
            ...
            StringUtils::contains($this->asset->publicId(), '?')
        ) {
            return $urlParts;
        }
