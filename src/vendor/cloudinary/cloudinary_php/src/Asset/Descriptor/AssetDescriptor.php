...
class AssetDescriptor implements AssetInterface
{
    ...
    public function setPublicId($publicId)
    {
        list(
            ...
            $this->filename,
            $this->extension,
        ) = FileUtils::splitPathFilenameExtension($publicId);
    ...
    public function setSuffix($suffix)
    {
        ...
        if (preg_match('/[.\/]/', $suffix)) {
            throw new \UnexpectedValueException(
                static::class . '::$suffix must not include . or /'
            );
        }
    ...
    {
        $assetJson = [
            'asset_type' => ArrayUtils::get(
                $params,
                'resource_type',
                AssetType::IMAGE
            ),
            'delivery_type' => ArrayUtils::get(
                $params,
                'type',
                DeliveryType::UPLOAD
            ),
    ...
    public function importJson($json)
    {
        ...
        if (
            !array_key_exists('asset', $json) ||
            !array_key_exists('filename', $json['asset'])
        ) {
        ...
        $this->assetType = ArrayUtils::get(
            $assetJson,
            'asset_type',
            AssetType::IMAGE
        );
        $this->deliveryType = ArrayUtils::get(
            $assetJson,
            'delivery_type',
            DeliveryType::UPLOAD
        );
