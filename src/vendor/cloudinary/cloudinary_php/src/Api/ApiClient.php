...
class ApiClient extends BaseApiClient
{
    ...
    {
        return $this->callAsync(HttpMethod::POST, $endPoint, [
            'body' => Utils::buildHttpQuery($formParams),
        ]);
    }
    ...
    public function postMultiPart(
        ...
        return $this->postMultiPartAsync(
            $endPoint,
            $multiPart,
            $headers,
            $options
    ...
    public function postFileAsync($endPoint, $file, $parameters, $options = [])
    {
        ...
        if ($fileHandle instanceof UriInterface) {
            return $this->postSingleChunkAsync(
                $endPoint,
                $fileHandle,
                $parameters,
                $options
        ...
        if (
            empty(ArrayUtils::get($options, 'filename')) &&
            FileUtils::isLocalFilePath($file)
        ) {
        ...
        $options[ApiConfig::CHUNK_SIZE] = ArrayUtils::get(
            $options,
            ApiConfig::CHUNK_SIZE,
            $this->api->chunkSize
        );

        $options[ApiConfig::TIMEOUT] = ArrayUtils::get(
            $options,
            ApiConfig::TIMEOUT,
            $this->api->uploadTimeout
        );
        ...
        return $this->postLargeFileAsync(
            $endPoint,
            $fileHandle,
            $parameters,
            $options
    ...
    private function postLargeFileAsync(
        ...
        $chunkSize = ArrayUtils::get(
            $options,
            ApiConfig::CHUNK_SIZE,
            ApiConfig::DEFAULT_CHUNK_SIZE
        );

        $fileSize = $fileHandle->getSize(); // Can be null as well
        /** @noinspection IsEmptyFunctionUsageInspection */
        $rangeHeaderTemplate =
            'bytes %s-%s' . (!empty($fileSize) ? "/$fileSize" : '');

        $options['headers'] = [
            'X-Unique-Upload-Id' => Utils::randomPublicId(
        ...
        while (!$fileHandle->eof()) {
            ...
            try {
                $this->getLogger()->debug(
                    'Making a Single Chunk Async POST request'
                );
                // TODO: use pool here or send requests in an async manner(?)
                $uploadResult = $this->postSingleChunkAsync(
                    $endPoint,
                    $currChunk,
                    $parameters,
                    $options
            ...
            ArrayUtils::addNonEmptyFromOther(
                $parameters,
                'public_id',
                $uploadResult
            );
    ...
    protected function buildHttpClientConfig()
    {
        ...
        if (isset($this->cloud->oauthToken)) {
            $authConfig = [
                'headers' => [
                    'Authorization' => 'Bearer ' . $this->cloud->oauthToken,
                ],
            ];
        } else {
            $authConfig = [
    ...
    {
        return array_values(
            ArrayUtils::mapAssoc(static function ($key, $value) {
