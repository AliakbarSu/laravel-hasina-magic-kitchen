...
class ApiUtils
{
    ...
    {
        return self::serializeParameter(
            $headers,
            self::HEADERS_OUTER_DELIMITER,
            self::HEADERS_INNER_DELIMITER
        );
    }
    ...
    public static function serializeContext($context)
    {
        if (is_array($context)) {
            $context = array_map(
                '\Cloudinary\Api\ApiUtils::serializeJson',
                $context
            );
        }
        ...
        return self::serializeParameter(
            $context,
            self::CONTEXT_OUTER_DELIMITER,
            self::CONTEXT_INNER_DELIMITER
        );
    }
    ...
    public static function serializeJson($jsonParam)
    {
        ...
        if (
            is_string($jsonParam) ||
            (is_object($jsonParam) && method_exists($jsonParam, '__toString'))
        ) {
            return $jsonParam;
        }
     ...
     *
     */
    public static function serializeParameter(
        $parameter,
        $outerDelimiter = null,
        $innerDelimiter = null
    ) {
    ...
    public static function serializeArrayOfArrays($arrayOfArrays)
    {
        ...
        return self::serializeParameter(
            $array,
            self::ARRAY_OF_ARRAYS_DELIMITER
        );
    }
    ...
    public static function serializeAssetTransformations($transformations)
    {
        ...
        return ArrayUtils::implodeFiltered(
            self::ASSET_TRANSFORMATIONS_DELIMITER,
            $serializedTransformations
        );
    }
    ...
    {
        return (string) ClassUtils::forceInstance(
        ...
        foreach (ArrayUtils::build($breakpoints) as $breakpointSettings) {
            if ($breakpointSettings) {
                ...
                if ($transformation) {
                    $breakpointSettings[
                        'transformation'
                    ] = self::serializeAssetTransformations($transformation);
                }
    ...
    public static function signParameters(
        $parameters,
        $secret,
        $signatureAlgorithm = Utils::ALGO_SHA1
    ) {
        $parameters = array_map(
            self::class . '::serializeSimpleApiParam',
            $parameters
        );
