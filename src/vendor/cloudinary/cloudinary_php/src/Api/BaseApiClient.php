...
class BaseApiClient
{
    ...
    private static $userAgent =
        'CloudinaryPHP/' . Cloudinary::VERSION . ' (PHP ' . PHP_VERSION . ')';
    ...
    {
        return $this->callAsync(HttpMethod::GET, $endPoint, [
            'query' => Utils::buildHttpQuery($queryParams),
        ]);
    }
    ...
    {
        return $this->callAsync(HttpMethod::DELETE, $endPoint, [
            'form_params' => $fields,
        ])->wait();
    }
    ...
    {
        return $this->callAsync(HttpMethod::PUT, $endPoint, [
            'form_params' => $fields,
        ])->wait();
    }
    ...
    protected function callAsync($method, $endPoint, $options)
    {
        ...
        $this->getLogger()->debug("Making async $method request", [
            'method' => $method,
            'endPoint' => $endPoint,
        ]);
            ...
            ->then(
                function (ResponseInterface $response) {
                    ...
                    try {
                        ...
                        $this->getLogger()->critical('Async request failed', [
                            'code' => $e->getCode(),
                ...
                function (Exception $error) {
                    ...
                    if ($error instanceof ClientException) {
                        return Create::rejectionFor(
                            $this->handleApiResponse($error->getResponse())
                        );
                    }
    ...
    private function handleApiResponse($response)
    {
        ...
        if ($statusCode !== HttpStatusCode::OK) {
            ...
            $message =
                "Server returned unexpected status code - {$response->getStatusCode()} - " .
