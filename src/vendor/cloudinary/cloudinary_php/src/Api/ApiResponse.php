...
class ApiResponse extends ArrayObject
{
    ...
    public function __construct($responseJson, $headers)
    {
        ...
        $this->rateLimitResetAt = strtotime(
            ArrayUtils::get($lcHeaders, ['x-featureratelimit-reset', 0], 0)
        );
        $this->rateLimitAllowed = (int) ArrayUtils::get(
            $lcHeaders,
            ['x-featureratelimit-limit', 0],
            0
        );
        $this->rateLimitRemaining = (int) ArrayUtils::get(
            $lcHeaders,
