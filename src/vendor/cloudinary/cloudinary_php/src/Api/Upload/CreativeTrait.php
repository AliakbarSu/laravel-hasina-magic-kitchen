...
trait CreativeTrait
{
    ...
    public function generateSpriteAsync($tag, $options = [])
    {
        ...
        return $this->callUploadApiJsonAsync(
            UploadEndPoint::SPRITE,
            $params,
            $options
        );
    }
    ...
    {
        $options['transformation'] = ApiUtils::serializeAssetTransformations(
            $options
        );
    ...
    private static function buildSpriteAndMultiParams($tagOrOptions, $options)
    {
        ...
        if (empty($tag) && empty($urls)) {
            throw new InvalidArgumentException(
                'Either tag or urls are required'
            );
        }
    ...
    public function createSlideshowAsync($options = [])
    {
        ...
        $complexParams = [
            'manifest_transformation' => ApiUtils::serializeAssetTransformations(
                ArrayUtils::get($options, 'manifest_transformation')
            ),
            'manifest_json' => ApiUtils::serializeJson(
                ArrayUtils::get($options, 'manifest_json')
            ),
