...
class SrcSet
{
    ...
    {
        $this->image = ClassUtils::forceInstance(
            $image,
            Image::class,
            null,
            $configuration
        ...
        $this->responsiveBreakpointsConfig =
            $configuration->responsiveBreakpoints;
     ...
     * @return array
     */
    private function calculateAutoOptimalBreakpoints(
        $minWidth,
        $maxWidth,
        $maxImages
    ) {
        list($minWidth, $maxWidth, $maxImages) = $this->validateInput(
            $minWidth,
            $maxWidth,
            $maxImages
        );
        ...
        list(
            $physicalMinWidth,
            $physicalMaxWidth,
        ) = self::getPhysicalDimensions($minWidth, $maxWidth);
    ...
    protected static function getPhysicalDimensions($minWidth, $maxWidth)
    {
        ...
        if ($physicalMinWidth > $physicalMaxWidth) {
            list($physicalMinWidth, $physicalMaxWidth) = [
                $physicalMaxWidth,
                $physicalMinWidth,
            ];
        }
    ...
    {
        return $dimension < self::DEFAULT_DPR_THRESHOLD
            ? (int) $dimension * self::DEFAULT_DPR
            : $dimension;
    }
    ...
    protected static function collectFromGroup($group, $whitelist, $count = 1)
    {
        ...
        foreach ($group as $res) {
            if (
                in_array($res, $whitelist, false) &&
                !in_array($res, $result, false)
            ) {
                $result[] = $res;
                $count--;
                if (!$count) {
    ...
    private function validateInput($minWidth, $maxWidth, $maxImages)
    {
        ...
        foreach ([$minWidth, $maxWidth, $maxImages] as $arg) {
            if (empty($arg) || !is_numeric($arg) || is_string($arg)) {
                $message =
                    'Either valid (minWidth, maxWidth, maxImages) or breakpoints' .
