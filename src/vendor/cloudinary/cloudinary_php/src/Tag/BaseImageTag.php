 ...
 * @api
 */
abstract class BaseImageTag extends BaseTag implements
    ImageTransformationInterface
{
    ...
    public static function fromParams($source, $params = [])
    {
        ...
        if (
            array_key_exists('srcset', $params) &&
            is_array($params['srcset'])
        ) {
            if (
                array_key_exists('sizes', $params['srcset']) &&
                $params['srcset']['sizes']
            ) {
        ...
        return (new static($image, $configuration))->setAttributes(
            $tagAttributes
        );
    }
    ...
    public static function handleResponsive(&$params, $configuration)
    {
        ...
        $configuration->tag->clientHints = ArrayUtils::pop(
            $params,
            'client_hints',
            $configuration->tag->clientHints
        );
    }
