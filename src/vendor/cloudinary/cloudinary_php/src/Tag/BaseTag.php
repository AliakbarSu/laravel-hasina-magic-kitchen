...
abstract class BaseTag
{
    ...
    {
        $closingChar =
            static::IS_VOID && $this->config->tag->voidClosingSlash
                ? '/>'
                : '>';
    ...
    public function serializeContent(
        ...
        $content = $prependAdditionalContent
            ? ArrayUtils::mergeNonEmpty($additionalContent, $this->content)
            : ArrayUtils::mergeNonEmpty($this->content, $additionalContent);

        return implode($this->config->tag->contentDelimiter, $content);
    }
    ...
    public function serializeAttributes($attributes = [])
    {
        ...
        if (
            $this->config->tag->prependSrcAttribute &&
            array_key_exists('src', $allAttributes)
        ) {
            ArrayUtils::prependAssoc(
                $allAttributes,
                'src',
                ArrayUtils::pop($allAttributes, 'src')
            );
        }
