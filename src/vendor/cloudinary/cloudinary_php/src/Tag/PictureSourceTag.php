...
class PictureSourceTag extends BaseImageTag
{
    ...
    public function sizes($sizes = null)
    {
        if (
            $sizes &&
            $this->config->responsiveBreakpoints->autoOptimalBreakpoints
        ) {
            throw new \InvalidArgumentException(
                'Invalid input: sizes must not be used with autoOptimalBreakpoints'
            );
        }
    ...
    public function serializeAttributes($attributes = [])
    {
        if (!empty($this->srcset->getBreakpoints())) {
            ...
            $attributes['srcset'] = $this->image->toUrl(
                $this->additionalTransformation
            );
        }
