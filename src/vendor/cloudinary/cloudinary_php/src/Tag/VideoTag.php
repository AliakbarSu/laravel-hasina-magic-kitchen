...
class VideoTag extends BaseTag implements VideoTransformationInterface
{
            ...
            if (is_array($source)) {
                $sourceTag = new VideoSourceTag(
                    ...
                    ArrayUtils::get(
                        $source,
                        'transformation',
                        ArrayUtils::get($source, 'transformations')
                    )
                );
                $sourceTag->type(
                    ArrayUtils::get($source, 'type'),
                    ArrayUtils::get($source, 'codecs')
                );
    ...
    public static function fromParams($source, $params = [])
    {
        ...
        $sourceTypes = ArrayUtils::pop(
            $params,
            'source_types',
            self::$defaultSourceTypes
        );
        ...
        if (count($sources) === 1) {
            $video->asset->setPublicId(
                $video->asset->publicId(true) . '.' . $sources[0]['type']
            );
            $sources = [];
        }
        ...
        $tagAttributes['poster'] = self::generateVideoPosterAttr(
            $source,
            $params,
            $configuration
        );
        ...
        TagUtils::handleSpecialAttributes(
            $tagAttributes,
            $params,
            $configuration
        );
        $tagAttributes = array_merge(
            $tagAttributes,
            self::collectAttributesFromParams($params)
        );

        return (new static($video, $sources, $configuration))
     ...
     *                     * @internal
     */
    protected static function generateVideoPosterAttr(
        $source,
        &$videoParams,
        $configuration
    ) {
        ArrayUtils::setDefaultValue(
            $videoParams,
            'resource_type',
            AssetType::VIDEO
        );
        ArrayUtils::setDefaultValue(
            $videoParams,
            'format',
            $configuration->tag->videoPosterFormat
        );
    ...
    {
        $sourceTransformation = ArrayUtils::pop(
            $params,
    ...
    public function serializeContent(
        $additionalContent = [],
        $prependAdditionalContent = false
    ) {
        ...
        return parent::serializeContent($content, true);
    }
    ...
    {
        return $this->applyAssetModification(
            'addTransformation',
            $transformation
        );
    }
