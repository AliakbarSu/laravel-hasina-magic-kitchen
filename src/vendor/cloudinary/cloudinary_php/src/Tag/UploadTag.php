...
class UploadTag extends BaseConfigurableApiTag
{
    ...
    public static function fromParams($field, $params = [])
    {
        ...
        return (new self($field, $configuration, $params))->addClass(
            ArrayUtils::get($params, ['html', 'class'])
        );
    }
    ...
    public function serializeAttributes($attributes = [])
    {
        ...
        $attributes['data-form-data'] = JsonUtils::encode(
            $this->getUploadParams()
        );
        $attributes['data-url'] = $this->uploadApi->getUploadUrl(
            $this->assetType
        );
    ...
    public static function unsigned(
        ...
        $tag = new UploadTag($field, $configuration, $uploadParams, $assetType);
