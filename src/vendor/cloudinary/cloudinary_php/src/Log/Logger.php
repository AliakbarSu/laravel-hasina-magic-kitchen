...
class Logger
{
        ...
        try {
            ...
            if (is_array($config->file)) {
                foreach ($config->file as $loggingFileConfig) {
                    if (
                        empty($loggingFileConfig['path']) ||
                        !is_string($loggingFileConfig['path'])
                    ) {
                        continue;
                    }
                    $logLevel = ArrayUtils::get(
                    ...
                    $this->addHandler(
                        new StreamHandler($loggingFileConfig['path'], $logLevel)
                    );
                }
            }
    ...
    private function addHandler(HandlerInterface $handler)
    {
        foreach ($this->entity->getHandlers() as $entityHandler) {
            if (
                $entityHandler instanceof HandlerInterface &&
    ...
    public function getDefaultLogLevel()
    {
        ...
        $pot = 0;
        foreach (
            array_reverse(str_split(decbin($phpErrorReportingLevel)))
            as $bit
        ) {
            if ($bit === 1) {
                $this->defaultLogLevel = min(
                    $this->defaultLogLevel,
                    $this->errorLevelMap[2 ** $pot]
                );
            }
            $pot++;
        }
