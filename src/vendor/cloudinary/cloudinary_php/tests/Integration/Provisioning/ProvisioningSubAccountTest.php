...
final class ProvisioningSubAccountTest extends ProvisioningIntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$SUB_ACCOUNT_CREATE_NAME =
            'provisioning_sub_account_name_create_' . self::$UNIQUE_TEST_ID;
        self::$SUB_ACCOUNT_CREATE_2_NAME =
            'provisioning_sub_account_name_create_2_' . self::$UNIQUE_TEST_ID;
        self::$SUB_ACCOUNT_CREATE_3_NAME =
            'provisioning_sub_account_name_create_3_' . self::$UNIQUE_TEST_ID;
        self::$SUB_ACCOUNT_CREATE_CLOUD_NAME =
            'cloud' . str_replace('_', '-', self::$SUB_ACCOUNT_CREATE_NAME);
        self::$SUB_ACCOUNT_DELETE_NAME =
            'provisioning_sub_account_name_delete_' . self::$UNIQUE_TEST_ID;
        self::$SUB_ACCOUNT_GET_NAME =
            self::$PREFIX .
            'provisioning_user_name_get_' .
            self::$UNIQUE_TEST_ID;
        self::$SUB_ACCOUNT_UPDATE_NAME =
            'provisioning_user_name_update_' . self::$UNIQUE_TEST_ID;
        self::$SUB_ACCOUNT_UPDATE_NEW_NAME =
            'new_' . self::$SUB_ACCOUNT_UPDATE_NAME;
        self::$SUB_ACCOUNT_UPDATE_NEW_CLOUD_NAME =
            'cloud' . str_replace('_', '-', self::$SUB_ACCOUNT_UPDATE_NEW_NAME);
        self::$SUB_ACCOUNT_CUSTOM_ATTRIBUTES = [
            'key_1_' . self::$UNIQUE_TEST_ID =>
                'value_1_' . self::$UNIQUE_TEST_ID,
            'key_2_' . self::$UNIQUE_TEST_ID =>
                'value_2_' . self::$UNIQUE_TEST_ID,
        ];
        self::$SUB_ACCOUNT_NEW_CUSTOM_ATTRIBUTES = [
            'key_3_' . self::$UNIQUE_TEST_ID =>
                'value_3_' . self::$UNIQUE_TEST_ID,
        ];
        ...
        self::assertValidSubAccountUser($subAccount, [
        ...
        self::$SUB_ACCOUNTS[] = self::$SUB_ACCOUNT_DELETE_ID =
            $subAccount['id'];

        self::assertValidSubAccountUser($subAccount, [
            'name' => self::$SUB_ACCOUNT_DELETE_NAME,
        ]);
        ...
        self::$SUB_ACCOUNTS[] = self::$SUB_ACCOUNT_UPDATE_ID =
            $subAccount['id'];

        self::assertValidSubAccountUser($subAccount, [
            'name' => self::$SUB_ACCOUNT_UPDATE_NAME,
        ]);
    }
    ...
    {
        $result = self::$accountApi->subAccounts(null, [
            self::$SUB_ACCOUNTS[0],
            self::$SUB_ACCOUNTS[1],
        ]);
    ...
    public function testCreateSubAccountBasedOnExistingAccountWithCustomAttributes()
    {
        self::markTestSkipped(
            'The custom attributes is not copied from a base sub account'
        );
    ...
    {
        $result = self::$accountApi->deleteSubAccount(
            self::$SUB_ACCOUNT_DELETE_ID
        );
