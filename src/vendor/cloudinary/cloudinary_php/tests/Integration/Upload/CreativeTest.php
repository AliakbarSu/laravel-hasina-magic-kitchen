...
final class CreativeTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$TAG_TO_GENERATE_SPRITE =
            'upload_creative_generate_sprite_' . self::$UNIQUE_TEST_TAG;

        self::$TRANSFORMATION_STRING = (string) new Transformation(
            self::TRANSFORMATION
        );

        $tags = [self::$TAG_TO_GENERATE_SPRITE, self::$TAG_TO_MULTI];

        self::createTestAssets([
            ['options' => ['tags' => $tags]],
            ['options' => ['tags' => $tags]],
            self::EXPLODE_GIF => [
                'options' => ['file' => self::TEST_IMAGE_GIF_PATH],
            ],
            self::SPRITE_TEST_1 => [
                'option' => ['tags' => self::$TAG_TO_GENERATE_SPRITE],
            ],
            self::SPRITE_TEST_2 => [
                'option' => ['tags' => self::$TAG_TO_GENERATE_SPRITE],
            ],
    ...
    public function testGenerateSprite()
    {
        ...
        self::addAssetToCleanupList($asset, [
            DeliveryType::KEY => DeliveryType::SPRITE,
        ]);
    ...
    {
        $urlFromTag = self::$uploadApi->downloadGeneratedSprite(
            self::$TAG_TO_GENERATE_SPRITE
        );
        $urlFromUrls = self::$uploadApi->downloadGeneratedSprite([
            'urls' => [self::URL_1, self::URL_2],
        ]);
        ...
        self::assertDownloadSignUrl(
            ...
            '/' .
                ...
                '/' .
                Configuration::instance()->cloud->cloudName .
                '/image/sprite',
            [
    ...
    public function testCreateMulti()
    {
        ...
        self::addAssetToCleanupList($asset, [
            DeliveryType::KEY => DeliveryType::MULTI,
        ]);
