...
final class EditTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$RENAME_RETURN_CONTEXT_KEY =
            'rename_return_context_key_' . self::$UNIQUE_TEST_ID;
        self::$RENAME_RETURN_CONTEXT_VALUE =
            'rename_return_context_value_' . self::$UNIQUE_TEST_ID;
        self::$RENAME_RETURN_METADATA_EXTERNAL_ID =
            'rename_metadata_external_id_' . self::$UNIQUE_TEST_ID;
        self::$RENAME_RETURN_METADATA_DEFAULT_VALUE =
            'rename_metadata_default_value_' . self::$UNIQUE_TEST_ID;

        self::$RENAME_RETURN_METADATA = new StringMetadataField(
            self::$RENAME_RETURN_METADATA_EXTERNAL_ID
        );
        self::$RENAME_RETURN_METADATA->setExternalId(
            self::$RENAME_RETURN_METADATA_EXTERNAL_ID
        );
        self::$RENAME_RETURN_METADATA->setDefaultValue(
            self::$RENAME_RETURN_METADATA_DEFAULT_VALUE
        );
    ...
    public static function tearDownAfterClass()
    {
        ...
        self::$adminApi->deleteMetadataField(
            self::$RENAME_RETURN_METADATA_EXTERNAL_ID
        );
    ...
    {
        $result = self::$uploadApi->destroy(
            self::getTestAssetPublicId(self::DESTROY)
        );
    ...
    public function testRenameImage()
    {
        ...
        self::assertValidAsset($asset, [
            'public_id' => self::getTestAssetPublicId(self::RENAME_TARGET),
        ]);
    ...
    public function testRenameAndOverwriteToExistingImagePublicId()
    {
        ...
        self::assertValidAsset($resource, [
            'public_id' => self::getTestAssetPublicId(
                self::RENAME_TO_EXISTING_TARGET_OVERWRITE
            ),
        ]);

        $resource = self::$adminApi->asset(
            self::getTestAssetPublicId(
                self::RENAME_TO_EXISTING_TARGET_OVERWRITE
            )
        );
    ...
    public function testChangeImageType()
    {
        ...
        self::assertRequestBodySubset($lastRequest, [
            'to_type' => 'private',
    ...
    public function testEagerTransformation()
    {
        $asset = self::$uploadApi->explicit(
            ...
            [
                'type' => 'upload',
                'eager' => [self::$TRANSFORMATION_OBJECT],
            ]
        );

        self::assertValidAsset($asset);
        self::assertCount(1, $asset['eager']);
        self::assertValidTransformationRepresentation($asset['eager'][0], [
        ...
        self::assertValidDerivedAsset($asset['derived'][0], [
            'transformation' => self::TRANSFORMATION_STRING,
            'format' => AssetTestCase::IMG_EXT_GIF,
            'bytes' => 43,
        ]);
