...
final class UploadApiTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$METADATA_FIELD_UNIQUE_EXTERNAL_ID =
            'metadata_field_external_id_' . self::$UNIQUE_TEST_ID;
        self::$METADATA_FIELD_VALUE =
            'metadata_field_value_' . self::$UNIQUE_TEST_ID;

        self::$METADATA_FIELD_EXTERNAL_ID_SET =
            'metadata_external_id_uploader_set_' . self::$UNIQUE_TEST_ID;
        self::$DATASOURCE_ENTRY_EXTERNAL_ID =
            'metadata_datasource_entry_external_id' . self::$UNIQUE_TEST_ID;
        self::$DATASOURCE_ENTRY_EXTERNAL_ID2 =
            'metadata_datasource_entry_external_id2' . self::$UNIQUE_TEST_ID;
        ...
        $stringMetadataField->setExternalId(
            self::$METADATA_FIELD_UNIQUE_EXTERNAL_ID
        );
        ...
        self::$INCOMING_TRANSFORMATION_ARR = [
            'crop' => 'scale',
            'width' => self::INCOMING_TRANSFORMATION_WIDTH,
        ];
        self::$INCOMING_TRANSFORMATION_OBJ = Resize::scale(
            self::INCOMING_TRANSFORMATION_WIDTH
        );
    }
    ...
    {
        $result = self::$uploadApi->upload(self::TEST_IMAGE_PATH, [
            'tags' => self::$ASSET_TAGS,
        ]);
    ...
    {
        $p = self::$uploadApi->uploadAsync(self::TEST_IMAGE_PATH, [
            'tags' => self::$ASSET_TAGS,
        ]);
    ...
    {
        $result = self::$uploadApi->upload(self::TEST_BASE64_IMAGE, [
            'filename' => self::$UNIQUE_TEST_ID,
            'tags' => self::$ASSET_TAGS,
        ]);
    ...
    {
        $result = self::$uploadApi->upload(AssetTestCase::FETCH_IMAGE_URL, [
            'tags' => self::$ASSET_TAGS,
        ]);

        self::assertValidAsset($result, [
            'original_filename' => AssetTestCase::ASSET_ID,
        ]);

        $result = self::$uploadApi->upload(
            new Uri(AssetTestCase::FETCH_IMAGE_URL),
            ['tags' => self::$ASSET_TAGS]
        );
    ...
    public function testUploadGSUrl()
    {
        ...
        $gsImage =
            'gs://gcp-public-data-landsat/LC08/PRE/044/034/LC80440342016259LGN00/LC80440342016259LGN00_BQA.TIF';
        $result = self::$uploadApi->upload($gsImage, [
            'tags' => self::$ASSET_TAGS,
        ]);
    ...
    {
        $result = self::$uploadApi->upload(self::populateLargeImage(), [
    ...
    public function testUploadNonSeekableStream()
    {
        ...
        $result = self::$uploadApi->upload($stream, [
        ...
        self::assertValidAsset($result, [
            'original_filename' => self::$REMOTE_TEST_IMAGE_ID,
        ]);
    }
    ...
    protected static function populateLargeImage()
    {
        ...
        $largeStream = Psr7\Utils::streamFor(
            $head .
                str_repeat("\xFF", self::LARGE_TEST_IMAGE_SIZE - strlen($head))
        );
        $largeStream->rewind();
    ...
    {
        $result = self::uploadTestAssetImage(
            self::$INCOMING_TRANSFORMATION_ARR
        );
        ...
        $result = self::uploadTestAssetImage([
            'transformation' => self::$INCOMING_TRANSFORMATION_OBJ,
        ]);
    ...
    {
        $result = self::uploadTestAssetImage(
            array_merge(self::$INCOMING_TRANSFORMATION_ARR, ['format' => 'gif'])
        );
    ...
    {
        $asset = self::$uploadApi->upload(self::TEST_IMAGE_PATH, [
            'tags' => self::$ASSET_TAGS,
    ...
    public function testExplicitWithMetadata()
    {
        ...
        $result = self::$uploadApi->explicit($asset['public_id'], [
    ...
    public function testUploaderUpdateMetadata()
    {
        ...
        $result = self::$uploadApi->updateMetadata(self::$METADATA_FIELDS, [
            $asset['public_id'],
        ]);
    ...
    {
        $resource1 = self::$uploadApi->upload(self::TEST_IMAGE_PATH, [
            'tags' => self::$ASSET_TAGS,
        ]);
        $resource2 = self::$uploadApi->upload(self::TEST_IMAGE_PATH, [
            'tags' => self::$ASSET_TAGS,
        ]);
    ...
    public function testEvalUploadParameter()
    {
        self::retryAssertionIfThrows(
            function () {
                ...
                self::assertValidAsset($result, [
                    'context' => [
                        'custom' => ['width' => self::TEST_IMAGE_WIDTH],
                    ],
                ]);
    ...
    public function testResponsiveBreakpointsFormat()
    {
        ...
        self::assertEquals(
            'a_90',
            $asset['responsive_breakpoints'][0]['transformation']
        );
        self::assertMatchesRegularExpression(
            '/\.gif$/',
            $asset['responsive_breakpoints'][0]['breakpoints'][0]['url']
        );
        self::assertMatchesRegularExpression(
            '/\.gif$/',
