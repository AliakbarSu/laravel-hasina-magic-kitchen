...
abstract class IntegrationTestCase extends CloudinaryTestCase
{
    ...
    const TEST_IMAGE_GIF_PATH =
        self::TEST_ASSETS_DIR . AssetTestCase::IMAGE_NAME_GIF;
    ...
    {
        $cldRunDestructiveTests = strtolower(
            getenv('CLD_RUN_DESTRUCTIVE_TESTS')
        );
    ...
    protected static function createTestAssets($assets = [], $prefix = null)
    {
        foreach ($assets as $key => $values) {
            ...
            $assetOptions = is_array($options)
                ? array_merge($assetOptions, $options)
                : $assetOptions;
            $assetType = ArrayUtils::get(
                $assetOptions,
                AssetType::KEY,
                AssetType::IMAGE
            );
    ...
    {
        $options['tags'] =
            isset($options['tags']) && is_array($options['tags'])
    ...
    protected static function uploadTestAssetVideo($options = [])
    {
        ...
        return self::uploadTestAsset(
            self::TEST_ASSETS_DIR . 'sample.mp4',
            $options
        );
    }
     ...
     */
    private static function addAssetToTestAssetsList(
        $asset,
        $options = [],
        $cleanup = false,
        $key = null
    ...
    {
        return isset(self::$TEST_ASSETS[$name]['asset'])
            ? self::$TEST_ASSETS[$name]['asset']
            : null;
    }
    ...
    private static function getUniquePublicId($name, $prefix = null)
    {
        ...
        return StringUtils::camelCaseToSnakeCase(
            $prefix . '_' . $name . '_' . self::$UNIQUE_TEST_ID
        );
    }
    ...
    protected static function assertValidAsset($asset, $values = [])
    {
        ...
        self::assertObjectStructure($asset, [
     ...
     * @param array        $values
     */
    protected static function assertValidArchive(
        $archive,
        $format = 'zip',
        $values = []
    ) {
        ...
        self::assertObjectStructure($archive, [
     ...
     * @param array        $values
     */
    protected static function assertValidTransformationRepresentation(
        $asset,
        $values = []
    ) {
        self::assertObjectStructure($asset, [
    ...
    {
        self::assertObjectStructure($resource, [
            'url' => IsType::TYPE_STRING,
            'secure_url' => IsType::TYPE_STRING,
    ...
    protected static function assertValidSprite($resource, $values = [])
    {
        ...
        foreach ($resource['image_infos'] as $imageInfo) {
            self::assertObjectStructure($imageInfo, [
                'width' => IsType::TYPE_INT,
                'height' => IsType::TYPE_INT,
                'x' => IsType::TYPE_INT,
                'y' => IsType::TYPE_INT,
     ...
     * @param array  $values
     */
    protected static function assertDownloadSignUrl(
        $assetUrl,
        $prefixUrl = null,
        $path = null,
        $values = []
    ...
    protected static function assertAssetDeleted(
        ...
        self::assertCount(
            $deletedCount + $notFoundCount,
            $result['deleted_counts']
        );
        self::assertEquals(
            $originalCount,
            $result['deleted_counts'][$publicId]['original']
        );
        self::assertEquals(
            $derivedCount,
            $result['deleted_counts'][$publicId]['derived']
        );
        if ($notFoundCount) {
     ...
     */
    protected static function assertValidUploadPreset(
        $uploadPreset,
        $values = [],
        $settings = []
    ) {
        self::assertNotEmpty($uploadPreset);
        self::assertObjectStructure($uploadPreset, [
    ...
    {
        self::assertObjectStructure($folder, [
            'name' => IsType::TYPE_STRING,
            'path' => IsType::TYPE_STRING,
        ]);
     ...
     * @param array        $values
     */
    protected static function assertValidStreamingProfile(
        $streamingProfile,
        $values = []
    ) {
        ...
        self::assertObjectStructure($streamingProfile['data'], [
        ...
        self::assertNotEmpty(
            $streamingProfile['data']['representations'][0]['transformation']
        );
     ...
     */
    protected static function assertValidTransformation(
        $transformation,
        $values = [],
        $transformationInfo = []
    ) {
        self::assertObjectStructure($transformation, [
     ...
     */
    protected static function assertValidUploadMapping(
        $uploadMapping,
        $values = [],
        $message = ''
    ) {
        self::assertObjectStructure(
            $uploadMapping,
            [
                'template' => IsType::TYPE_STRING,
                'folder' => IsType::TYPE_STRING,
            ],
            $message
        );
    ...
    protected static function cleanupAssetsByTag($tag, $options = [])
    {
        self::cleanupSoftly(
            ...
            static function ($result) use ($tag) {
                return !isset($result['deleted'][$tag]) ||
                    $result['deleted'][$tag] !== 'deleted';
            },
            $tag,
            $options
        );
    ...
    protected static function cleanupAsset($publicId, $options = [])
    {
        self::cleanupSoftly(
            ...
            static function ($result) use ($publicId) {
                return !isset($result['deleted'][$publicId]) ||
     ...
     */
    protected static function cleanupTestAssets(
        ...
        self::cleanupSoftly(
            ...
            static function ($result) use ($path) {
                return !isset($result['deleted']) ||
                    !in_array($path, $result['deleted'], true);
            },
            $path
        );
    }
     ...
     * @param array        $options
     */
    protected static function cleanupTransformation(
        $transformation,
        $options = []
    ) {
        self::cleanupSoftly(
            ...
            'Transformation ' .
                $transformation .
                ' deletion failed during teardown',
