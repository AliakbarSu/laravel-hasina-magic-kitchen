...
final class FoldersTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$FOLDER_NAME =
            self::$FOLDER_BASE_NAME . '_' . self::$UNIQUE_TEST_ID;
        self::$FOLDER2_NAME =
            self::$FOLDER_BASE_NAME . '_2_' . self::$UNIQUE_TEST_ID;
        self::$SUB_FOLDER_NAME = 'test_sub_folder_' . self::$UNIQUE_TEST_ID;
        self::$SUB_FOLDER_CREATE_NAME =
            'test_sub_folder_for_create_' . self::$UNIQUE_TEST_ID;
        self::$SUB_FOLDER_DELETE_NAME =
            'test_sub_folder_for_delete_' . self::$UNIQUE_TEST_ID;
        self::$SUB_FOLDER_FULL_PATH =
            self::$FOLDER_NAME . '/' . self::$SUB_FOLDER_NAME;
        self::$SUB_FOLDER_CREATE_FULL_PATH =
            self::$FOLDER_NAME . '/' . self::$SUB_FOLDER_CREATE_NAME;
        self::$SUB_FOLDER_DELETE_FULL_PATH =
            self::$FOLDER_NAME . '/' . self::$SUB_FOLDER_DELETE_NAME;
        self::$SUB_FOLDER_SINGLE_FULL_PATH =
            self::$SUB_FOLDER_FULL_PATH . '/' . self::$SUB_FOLDER_NAME;
    ...
    public function testListRootFolders()
    {
        ...
        self::assertObjectStructure($result, [
            'total_count' => IsType::TYPE_INT,
        ]);
        ...
        self::assertObjectStructure($result, [
            'total_count' => IsType::TYPE_INT,
            'next_cursor' => IsType::TYPE_STRING,
        ]);
        self::assertValidFolder($result['folders'][0]);
    }
    ...
    public function testListSubFolders()
    {
        ...
        $result = self::$adminApi->subfolders(self::$FOLDER_NAME, [
            'max_results' => 1,
        ]);
    ...
    {
        $result = self::$adminApi->deleteFolder(
            self::$SUB_FOLDER_DELETE_FULL_PATH
        );
    ...
    private static function createTestFolder($path)
    {
        ...
        self::retryAssertionIfThrows(static function () use (
            $path,
            $name,
            $pathAsArray
        ) {
            ...
            self::assertArrayContainsArray($folders['folders'], [
                'name' => $name,
                'path' => $path,
            ]);
        });
