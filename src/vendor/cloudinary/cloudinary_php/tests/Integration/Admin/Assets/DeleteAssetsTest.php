...
final class DeleteAssetsTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$FULL_DELETE_PREFIX =
            self::CLASS_PREFIX . '_' . self::$DELETE_PREFIX;
        self::$UNIQUE_TEST_TAG_DELETE =
            self::CLASS_PREFIX . 'delete_' . self::$UNIQUE_TEST_TAG;
        self::$UNIQUE_TEST_TAG_DELETE_OPTIONS =
            self::CLASS_PREFIX . 'delete_by_options_' . self::$UNIQUE_TEST_TAG;
    ...
    {
        $result = self::$adminApi->assetsByTag(self::$UNIQUE_TEST_TAG_DELETE, [
            AssetType::KEY => AssetType::IMAGE,
        ]);

        self::assertEquals(
            AssetType::IMAGE,
            $result['resources'][0][AssetType::KEY]
        );

        $resultDeleting = self::$adminApi->deleteAssetsByTag(
            self::$UNIQUE_TEST_TAG_DELETE
        );
    ...
    public function testDeleteSingleAssetByPublicId()
    {
        $result = self::$adminApi->deleteAssets(
            self::getTestAssetPublicId(self::DELETE_SINGLE)
        );
    ...
    {
        $result = self::$adminApi->deleteAssetsByPrefix(
            self::$FULL_DELETE_PREFIX
        );

        self::assertAssetDeleted(
            $result,
            self::getTestAssetPublicId(self::$DELETE_PREFIX)
        );
    ...
    public function testDeleteDerivedImagesOnly()
    {
        ...
        self::assertValidTransformationRepresentation($result['eager'][0], [
    ...
    public function testDeleteAssetsByOptions()
    {
        ...
        self::assertAssetDeleted(
            $result,
            self::getTestAssetPublicId(self::PRIVATE_ASSET)
        );
        ...
        self::assertValidAsset($assets['resources'][0], [
            DeliveryType::KEY => DeliveryType::UPLOAD,
            AssetType::KEY => AssetType::RAW,
        ]);
    }
