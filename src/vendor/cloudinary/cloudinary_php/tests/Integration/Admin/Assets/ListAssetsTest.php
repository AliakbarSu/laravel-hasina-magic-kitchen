...
final class ListAssetsTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$FULL_UNIQUE_PREFIX =
            self::CLASS_PREFIX . '_' . self::$UNIQUE_PREFIX;
        self::$UNIQUE_CONTEXT_KEY =
            self::CLASS_PREFIX . 'context_key_' . self::$UNIQUE_TEST_ID;
        self::$UNIQUE_CONTEXT_VALUE =
            self::CLASS_PREFIX . 'context_value_' . self::$UNIQUE_TEST_ID;
        self::$UNIQUE_CONTEXT =
            self::$UNIQUE_CONTEXT_KEY . '=' . self::$UNIQUE_CONTEXT_VALUE;
        self::$UNIQUE_TEST_TAG_TO_ONE_IMAGE_ASSET =
            self::CLASS_PREFIX .
            'unique_tag_to_one_image_asset_' .
            self::$UNIQUE_TEST_TAG;
        self::$UNIQUE_ASSET_FOLDER =
            self::$FULL_UNIQUE_PREFIX . '_asset_folder';
        ...
        self::fetchRemoteTestAsset(AssetTestCase::TEST_SOCIAL_PROFILE_ID, [
    ...
    public function testListUploadedAssetsByAssetFolder()
    {
        if (!self::shouldTestFeature(Feature::DYNAMIC_FOLDERS)) {
            self::markTestSkipped(
                'Skipping ListUploadedAssetsByAssetFolder test.'
            );
        }
        $result = self::$adminApi->assetsByAssetFolder(
            self::$UNIQUE_ASSET_FOLDER
        );
    ...
    {
        $result = self::$adminApi->assetsByTag(self::$UNIQUE_TEST_TAG, [
            AssetType::KEY => AssetType::RAW,
            'tags' => true,
        ]);
    ...
    public function testListImagesByContextKey()
    {
        ...
        self::assertEquals(
            self::$UNIQUE_CONTEXT_VALUE,
            $result['resources'][0]['context']['custom'][
                self::$UNIQUE_CONTEXT_KEY
            ]
        );
    }
    ...
    public function testListImagesApprovedByWebPurify()
    {
        ...
        self::uploadTestAssetImage([
            ModerationType::KEY => ModerationType::WEBPURIFY,
        ]);
