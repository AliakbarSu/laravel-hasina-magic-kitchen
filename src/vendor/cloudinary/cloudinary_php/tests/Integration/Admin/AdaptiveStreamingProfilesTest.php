...
final class AdaptiveStreamingProfilesTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$STREAMING_PROFILES[] = self::$STREAMING_PROFILE_STATIC =
            'streaming_profile_static_' . $id;
        self::$STREAMING_PROFILES[] = self::$STREAMING_PROFILE_AS_STRING =
            'streaming_profile_as_string_' . $id;
        self::$STREAMING_PROFILES[] = self::$STREAMING_PROFILE_AS_ARRAY =
            'streaming_profile_as_array_' . $id;
        self::$STREAMING_PROFILES[] = self::$STREAMING_PROFILE_UPDATE =
            'streaming_profile_update_' . $id;
        self::$STREAMING_PROFILES[] = self::$STREAMING_PROFILE_DELETE =
            'streaming_profile_delete_' . $id;
        self::$STREAMING_PROFILES[] = self::$STREAMING_PROFILE_AS_TRANSFORMATION =
            'streaming_profile_transformation_' . $id;

        self::$STREAMING_PROFILE_DISPLAY_NAME =
            'streaming_profile_display_name_' . $id;

        self::$TRANSFORMATION_AS_OBJECT = (new Transformation())->addAction(
            Qualifier::height(1)
        );
    ...
    public function testListStreamingProfiles()
    {
        ...
        self::assertObjectStructure($result['data'][0], [
    ...
    {
        $result = self::$adminApi->getStreamingProfile(
            self::$STREAMING_PROFILE_STATIC
        );

        self::assertValidStreamingProfile($result, [
            'name' => self::$STREAMING_PROFILE_STATIC,
        ]);
        self::assertCount(2, $result['data']['representations']);
        self::assertArrayContainsArray($result['data']['representations'], [
    ...
    public function testCreateStreamingProfileFromRepresentationsString()
    {
        ...
        self::assertCount(
            1,
            $result['data']['representations'][0]['transformation']
        );
        self::assertArrayContainsArray($result['data']['representations'], [
        ...
        $result = self::$adminApi->createStreamingProfile(
            ...
            [
                ...
                'representations' => [
                    [
                        'transformation' =>
                            self::$SMALL_IMAGE_FILL_REPRESENTATION_ARRAY,
                    ],
                ],
            ]
        );
    ...
    public function testCreateStreamingProfileFromRepresentationsObject()
    {
        $result = self::$adminApi->createStreamingProfile(
            ...
            [
                'representations' => [[self::$TRANSFORMATION_AS_OBJECT]],
            ]
        );

        self::assertValidStreamingProfile($result, [
            'name' => self::$STREAMING_PROFILE_AS_TRANSFORMATION,
        ]);
        ...
        self::assertEquals(
            1,
            $result['data']['representations'][0]['transformation'][0]['height']
        );
    }
    ...
    public function testUpdateStreamingProfile()
    {
        ...
        $result = self::$adminApi->updateStreamingProfile(
            self::$STREAMING_PROFILE_UPDATE,
            [
                'representations' => [self::$BIG_IMAGE_REPRESENTATION_STRING],
            ]
        );
    ...
    public function testDeleteStreamingProfile()
    {
        ...
        self::assertValidStreamingProfile($result, [
            'name' => self::$STREAMING_PROFILE_DELETE,
        ]);

        $result = self::$adminApi->deleteStreamingProfile(
            self::$STREAMING_PROFILE_DELETE
        );
