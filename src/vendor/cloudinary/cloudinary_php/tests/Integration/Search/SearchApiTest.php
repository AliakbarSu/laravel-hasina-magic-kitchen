...
class SearchApiTest extends IntegrationTestCase
{
        ...
        self::createTestAssets([
            ...
            [
                'options' => [
                    'tags' => [self::$STRING_2],
                    'context' => [
                        self::CONTEXT_KEY => self::$STRING_WITH_UNDERSCORE,
                    ],
                    'folder' => self::$FOLDER_NAME,
                ],
            ],
    ...
    {
        $results = $this->search
            ->expression('tags:' . self::$UNIQUE_TEST_TAG)
            ->execute();
    ...
    public function testFindAssetsByExpressionWithTag()
    {
        $result = $this->search
            ->expression(
                'resource_type:image AND tags:' . self::$STRING_WITH_UNDERSCORE
            )
            ->withField('tags')
            ->maxResults(1)
            ->execute();
        ...
        try {
            ...
            if (
                StringUtils::contains(
                    $br->getMessage(),
                    'Your subscription plan does not support aggregations'
                )
            ) {
