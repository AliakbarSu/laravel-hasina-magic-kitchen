...
abstract class CloudinaryTestCase extends TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$TEST_TAG =
            'cloudinary_php_v' .
            str_replace(['.', '-'], '_', Cloudinary::VERSION);
    ...
    {
        self::assertEquals(
            self::objectToArray($object1),
            self::objectToArray($object2)
        );
    }
    ...
    {
        $result = array_filter($haystack, static function ($item) use (
            $needle
        ) {
        ...
        self::assertGreaterThanOrEqual(
            1,
            count($result),
            'The $haystack array does not contain the $needle array'
        );
    }
     ...
     */
    public static function assertOneOfInternalTypes(
        $expected,
        $actual,
        $message = ''
    ) {
     ...
     * @throws Exception
     */
    public static function retryAssertionIfThrows(
        $f,
        $retries = 3,
        $delay = 3,
        $message = ''
     ...
     */
    public static function assertContainsInstancesOf(
        $className,
        array $haystack
    ) {
        $instanceFound = false;
        ...
        self::assertTrue(
            $instanceFound,
            'The $haystack array does not contain an instance of ' . $className
        );
    }
    ...
    protected static function assertObjectLoggedMessage($obj, $message, $level)
    {
        ...
        self::assertTrue(
            ...
            sprintf(
                'Object %s did not log the message or logged it with a different level',
                get_class($obj)
            )
        );
    }
     ...
     * @param string       $message
     */
    protected static function assertObjectStructure(
        $asset,
        array $keys,
        $message = ''
    ) {
        foreach ($keys as $key => $type) {
            $value =
                is_object($asset) && property_exists($asset, $key)
                    ? $asset->{$key}
                    : $asset[$key];
    ...
    protected static function generateDataProvider(
        ...
        return array_map(static function ($value) use (
            $prefixValue,
            $suffixValue,
            $prefixMethod,
            $suffixMethod
            ...
            return [
                ...
                'method' => StringUtils::snakeCaseToCamelCase(
                    $prefixMethod . $value . $suffixMethod
                ),
            ];
        }, $array);
    }
