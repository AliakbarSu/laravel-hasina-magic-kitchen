...
final class ConfigUtilsTest extends UnitTestCase
{
    ...
    {
        $configArr = ConfigUtils::parseCloudinaryUrl(
            $this->cloudinaryUrl . '/' . $this::SECURE_CNAME
        );
    ...
    {
        $configArr = ConfigUtils::parseCloudinaryUrl(
            "$this->cloudinaryUrl?foo[bar]=value&foo[baz][qux]=quxval"
        );
    ...
    public function testParseLoggerConfigFromCloudinaryUrl()
    {
        $configArr = ConfigUtils::parseCloudinaryUrl(
            $this->cloudinaryUrl .
                '?logging[level]=CRITICAL&logging[file][my_debug_file][path]=logs/cloudinary.log' .
