...
class ConfigurationAccountTest extends ProvisioningUnitTestCase
{
    public function testAccountConfigFromUrl()
    {
        ...
        self::assertEquals(
            self::ACCOUNT_ID,
            $config->provisioningAccount->accountId
        );
        self::assertEquals(
            self::ACCOUNT_API_KEY,
            $config->provisioningAccount->provisioningApiKey
        );
        self::assertEquals(
            self::ACCOUNT_API_SECRET,
            $config->provisioningAccount->provisioningApiSecret
        );
    }
    ...
    public function testAccountConfigJsonSerialize()
    {
        ...
        $expectedJsonConfig =
            '{"provisioning_account":{"account_id":"' .
            self::ACCOUNT_ID .
