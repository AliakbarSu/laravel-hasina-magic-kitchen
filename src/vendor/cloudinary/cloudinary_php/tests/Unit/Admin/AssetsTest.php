...
final class AssetsTest extends UnitTestCase
{
    ...
    public function restoreDeletedAssetSpecificVersionDataProvider()
    {
        return [
            ...
            [
                ...
                'options' => [
                    'versions' => [
                        '9fa643e295b4dd1b293272f6bd9ec6ae',
                        'b4dd1b293272f6bd9fa643e2959ec6ae',
                    ],
                    'resource_type' => 'raw',
                    'type' => 'private',
                ],
     ...
     */
    public function testRestoreDeletedAssetSpecificVersion(
        $publicIds,
        $options,
        $url,
        $bodyFields
    ...
    public function testUpdateAssetFields()
    {
        ...
        self::assertRequestUrl(
            $lastRequest,
            '/resources/image/upload/' . self::$UNIQUE_TEST_ID
        );
    ...
    public function testRelatedAssets()
    {
        ...
        $mockAdminApi = new MockAdminApi();
        $mockAdminApi->addRelatedAssets(self::$UNIQUE_TEST_ID, $testIds);

        $lastRequest = $mockAdminApi->getMockHandler()->getLastRequest();

        self::assertRequestUrl(
            $lastRequest,
            '/resources/related_assets/image/upload/' . self::$UNIQUE_TEST_ID
        );
        self::assertRequestJsonBodySubset($lastRequest, [
            'assets_to_relate' => $testIds,
        ]);

        $mockAdminApi = new MockAdminApi();
        $mockAdminApi->deleteRelatedAssets(self::$UNIQUE_TEST_ID, $testIds);
    ...
    {
        $testAssetIds = [self::API_TEST_ASSET_ID2, self::API_TEST_ASSET_ID3];
        ...
        self::assertRequestUrl(
            $lastRequest,
            '/resources/related_assets/' . self::API_TEST_ASSET_ID
        );
        self::assertRequestJsonBodySubset($lastRequest, [
