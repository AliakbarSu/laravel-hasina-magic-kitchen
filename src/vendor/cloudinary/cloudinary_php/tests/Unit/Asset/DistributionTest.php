...
final class DistributionTest extends AssetTestCase
{
    ...
    {
        self::assertImageUrl(self::IMAGE_NAME, $this->image->secure(), [
            'protocol' => self::PROTOCOL_HTTPS,
        ]);
        self::assertImageUrl(self::IMAGE_NAME, $this->image->secure(false), [
    ...
    public function testSecureDistributionFromConfig()
    {
        ...
        self::assertImageUrl(self::IMAGE_NAME, new Image(self::IMAGE_NAME), [
            'protocol' => self::PROTOCOL_HTTPS,
        ]);
    ...
    public function testSecureAkamai()
    {
        self::assertImageUrl(
            ...
            [
                'protocol' => self::PROTOCOL_HTTPS,
                'hostname' =>
                    self::CLOUD_NAME . '-' . UrlConfig::DEFAULT_SHARED_HOST,
                'cloud_name' => null,
            ]
        );
    }
    ...
    {
        self::assertImageUrl(self::IMAGE_NAME, $this->image->privateCdn(), [
            'hostname' =>
                self::CLOUD_NAME . '-' . UrlConfig::DEFAULT_SHARED_HOST,
            'cloud_name' => null,
        ]);
    ...
    public function testCNameSubDomain()
    {
        self::assertImageUrl(
            self::IMAGE_NAME,
            $this->image
                ->cname(self::TEST_HOSTNAME)
                ->cdnSubdomain()
                ->secure(false),
            [
                'hostname' =>
                    'a' . self::EXPECTED_SHARD . '.' . self::TEST_HOSTNAME,
                'protocol' => 'http',
            ]
        );
    }
    ...
    public function testLongSignature()
    {
        ...
        self::assertImageUrl(
            's--RVsT3IpYGITMIc0RjCpde9T9Uujc2c1X--/' . self::IMAGE_NAME,
            $this->image
        );
    }
