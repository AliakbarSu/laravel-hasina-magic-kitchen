...
{
    const EXPECTED_VERSIONED_PATH =
        self::TEST_ASSET_VERSION_STR . '/' . self::AUTH_TOKEN_TEST_IMAGE;
    /**
    ...
    {
        $message =
            'Should not add token if authToken is globally set but null auth token is explicitly set ' .
            'and signed = true';
    ...
    public function testExplicitAuthTokenShouldOverrideGlobalSetting()
    {
        ...
        $expectedToken =
            '__cld_token__=st=222222222~exp=222222322~' .
    ...
    public function testShouldThrowWhenAclAndUrlAreMissing()
    {
        ...
        $this->expectExceptionMessage(
            'AuthToken must contain either acl or url property'
        );
