...
abstract class AssetTestCase extends UnitTestCase
{
    ...
    const FETCH_IMAGE_URL =
        'https://res.cloudinary.com/demo/image/upload/' . self::IMAGE_NAME;
    const FETCH_IMAGE_URL_WITH_QUERY =
    ...
    const PRIVATE_CDN_HOSTNAME =
        self::CLOUD_NAME . '-res.' . UrlConfig::DEFAULT_DOMAIN;
    ...
    const URL_SUFFIXED_IMAGE_NAME =
        self::URL_SUFFIXED_ASSET_ID . '.' . self::IMG_EXT;
    ...
    protected static function assertAssetUrl(
        ...
        if (!$privateCdn) {
            ...
            $hostAndCloud = ArrayUtils::implodeFiltered('-', [
                $cloudName,
                $hostname,
            ]);
        }
    ...
    protected static function assertAssetFromParamsUrl(
        ...
        $expectedPath = ArrayUtils::get(
            ...
            "$assetType/$deliveryType/" .
                ArrayUtils::implodeUrl([$path, $actualSource])
        );
    ...
    {
        $errorsThrown = 0;
        set_error_handler(static function () use (&$errorsThrown) {
            $errorsThrown++;
        ...
        self::assertEquals(
            1,
            $errorsThrown,
            'Failed assert that error was thrown'
        );
    }
}
