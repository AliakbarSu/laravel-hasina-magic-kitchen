...
class DoubleQuote extends PartParser
{
    public function parse(): Result
    {
        ...
        while (
            !$this->lexer->current->isA(EmailLexer::S_DQUOTE) &&
            !$this->lexer->current->isA(EmailLexer::S_EMPTY)
        ) {
            if (
                isset($special[$this->lexer->current->type]) &&
                $setSpecialsWarning
            ) {
            ...
            if (
                ...
                return new InvalidEmail(
                    new ExpectingATEXT('Expecting ATEXT between DQUOTE'),
        ...
        if (
            ...
            !$prev->isA(EmailLexer::S_BACKSLASH)
        ) {
            return new InvalidEmail(
                new ExpectingATEXT('Expecting ATEXT between DQUOTE'),
                $this->lexer->current->value
