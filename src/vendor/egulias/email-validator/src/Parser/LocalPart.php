...
class LocalPart extends PartParser
{
    ...
    public function parse(): Result
    {
        ...
        while (
            !$this->lexer->current->isA(EmailLexer::S_AT) &&
    ...
    protected function validateTokens(bool $hasComments): Result
    {
        if (isset(self::INVALID_TOKENS[$this->lexer->current->type])) {
            return new InvalidEmail(
                new ExpectingATEXT('Invalid token found'),
                $this->lexer->current->value
            );
        }
        return new ValidEmail();
    ...
    {
        return $this->lexer->current->isA(EmailLexer::S_DOT) &&
            $this->lexer->getPrevious()->isA(EmailLexer::S_EMPTY);
    }
    ...
    private function validateEscaping(): Result
    {
        ...
        if ($this->lexer->isNextToken(EmailLexer::GENERIC)) {
            return new InvalidEmail(
                new ExpectingATEXT('Found ATOM after escaping'),
                $this->lexer->current->value
            );
        }
