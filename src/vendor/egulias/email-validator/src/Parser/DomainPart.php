...
class DomainPart extends PartParser
{
    ...
    private function checkEndOfDomain(): Result
    {
        ...
        if ($prev->isA(EmailLexer::S_HYPHEN)) {
            return new InvalidEmail(
                new DomainHyphened('Hypen found at the end of the domain'),
                $prev->value
            );
        }
    ...
    {
        $thereIsNoDomain =
            $this->lexer->current->isA(EmailLexer::S_EMPTY) ||
    ...
    protected function doParseDomainPart(): Result
    {
        ...
        do {
            ...
            $notAllowedChars = $this->checkNotAllowedChars(
                $this->lexer->current
            );
            ...
            $exceptionsResult = $this->checkDomainPartExceptions(
                $prev,
                $hasComments
            );
    ...
    protected function parseDomainLiteral(): Result
    {
        try {
            ...
            return new InvalidEmail(
                new ExpectingDomainLiteralClose(),
                $this->lexer->current->value
            );
        }
    ...
    protected function checkDomainPartExceptions(
        ...
        if (
            $this->lexer->current->isA(EmailLexer::S_OPENBRACKET) &&
            $prev->type !== EmailLexer::S_AT
        ) {
            return new InvalidEmail(
                new ExpectingATEXT('OPENBRACKET not after AT'),
                $this->lexer->current->value
            );
        }

        if (
            $this->lexer->current->isA(EmailLexer::S_HYPHEN) &&
            $this->lexer->isNextToken(EmailLexer::S_DOT)
        ) {
            return new InvalidEmail(
                new DomainHyphened('Hypen found near DOT'),
                $this->lexer->current->value
            );
        }
        ...
        if (
            ...
            return new InvalidEmail(
                new ExpectingATEXT('Escaping following "ATOM"'),
                $this->lexer->current->value
            );
        }
    ...
    protected function validateTokens(bool $hasComments): Result
    {
        ...
        if (!isset($validDomainTokens[$this->lexer->current->type])) {
            return new InvalidEmail(
                new ExpectingATEXT(
                    'Invalid token in domain: ' . $this->lexer->current->value
                ),
                $this->lexer->current->value
            );
        }
