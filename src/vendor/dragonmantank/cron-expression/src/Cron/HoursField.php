...
class HoursField extends AbstractField
{
    ...
    public function isSatisfiedBy(
        ...
        if (
            $lastTransition !== null &&
            $lastTransition['ts'] > (int) $date->format('U') - 3600
        ) {
    ...
    public function getPastTransition(DateTimeInterface $date): ?array
    {
        ...
        foreach ($this->transitions as $transition) {
            ...
            if (
                $nextTransition !== null &&
                $transition['ts'] < $nextTransition['ts']
            ) {
                continue;
            }
        ...
        if (!$invert) {
            if ($originalHour >= $target) {
                ...
                if ($actualDay !== $originalDay + 1 && $actualHour !== 0) {
                    ...
                    $date = $this->timezoneSafeModify(
                        $date,
                        "+{$offsetChange} seconds"
                    );
                }
