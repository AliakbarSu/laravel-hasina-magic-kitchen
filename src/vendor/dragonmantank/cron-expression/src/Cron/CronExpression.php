...
class CronExpression
{
    ...
    public static function registerAlias(
        ...
        if (1 !== preg_match('/^@\w+$/', $shortcut)) {
            throw new LogicException(
                "The alias `$alias` is invalid. It must start with an `@` character and contain alphanumeric (letters, numbers, regardless of case) plus underscore (_)."
            );
        }
    ...
    public static function unregisterAlias(string $alias): bool
    {
        ...
        if (isset(self::MAPPINGS[$shortcut])) {
            throw new LogicException(
                "The alias `$alias` is a built-in alias; it can not be unregistered."
            );
        }
    ...
    protected function getRunDate(
        ...
        if (isset($parts[self::DAY]) && isset($parts[self::WEEKDAY])) {
            ...
            $domRunDates = $domExpression->getMultipleRunDates(
                $nth + 1,
                $currentTime,
                $invert,
                $allowCurrentDate,
            ...
            $dowRunDates = $dowExpression->getMultipleRunDates(
                $nth + 1,
                $currentTime,
                $invert,
                $allowCurrentDate,
        ...
        for ($i = 0; $i < $this->maxIterationCount; ++$i) {
            foreach ($parts as $position => $part) {
                ...
                if (false === strpos($part, ',')) {
                    ...
                    foreach (
                        ...
                        if (
                            $field->isSatisfiedBy($nextRun, $listPart, $invert)
                        ) {
                            $satisfied = true;
            ...
            if (
                ...
                $this->fieldFactory
                    ->getField(self::MINUTE)
                    ->increment(
                        $nextRun,
                        $invert,
                        $parts[self::MINUTE] ?? null
                    );
                continue;
            }
